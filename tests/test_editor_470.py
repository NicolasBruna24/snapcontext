...
class TestAnalisisImpacto(unittest.TestCase):
    ...
    def test_advierte_de_dependientes_en_modo_auto(self):
        ...
        with mock.patch.object(sc, "aviso") as avisos:
            resultado = editor._analizar_impacto_previo(["utils.py"], self.tmp, auto=True)
    ...
    def test_sin_dependientes_no_avisa(self):
        editor = ag.AgenteEditorPropio()
        with mock.patch.object(sc, "aviso") as avisos:
            resultado = editor._analizar_impacto_previo(["main.py"], self.tmp, auto=True)
    ...
    def test_menu_anadir_dependientes(self):
        ...
        with mock.patch("ui.preguntar_interactivo", return_value="s") as preg:
            resultado = editor._analizar_impacto_previo(["utils.py"], self.tmp, auto=False)
    ...
    def test_auto_nunca_pregunta_por_input(self):
        editor = ag.AgenteEditorPropio()
        with (
            mock.patch("builtins.input", side_effect=AssertionError("input() en modo auto")),
            mock.patch(
                "ui.preguntar_interactivo", side_effect=AssertionError("pregunta en modo auto")
            ),
        ):
            resultado = editor._analizar_impacto_previo(["utils.py"], self.tmp, auto=True)
        self.assertEqual(resultado, ["utils.py"])
    ...
    def test_ejecutar_cancela_si_el_usuario_aborta(self):
        ...
        with mock.patch.object(
            ag.AgenteEditorPropio, "_analizar_impacto_previo", return_value=None
        ):
...
class TestContextoSelectivo(unittest.TestCase):
    ...
    def test_archivo_grande_usa_resumen_ast_y_no_el_contenido_completo(self):
        ...
        prompt, truncado = ag._construir_prompt_edicion(
            "parche", "mejora la funcion_017", "grande.py", self.grande, "python", False
        )
    ...
    def test_sin_funcion_mencionada_usa_proximidad(self):
        prompt, truncado = ag._construir_prompt_edicion(
            "parche", "arregla lo que falla", "grande.py", self.grande, "python", False
        )
        self.assertTrue(truncado)
    ...
    def test_splicear_bloque_reemplaza_y_conserva(self):
        ...
        self.assertIsNone(sc._splicear_bloque(original, "def inexistente():\n    pass\n", "x\n"))
    ...
    def test_sobrescribir_truncado_reensambla_el_archivo(self):
        ...
        despues = antes.replace("return MARCA_003", "return MARCA_003 + 1000")
        respuesta = f"<<<ANTES>>>\n{antes}\n<<<DESPUES>>>\n{despues}\n<<<FIN>>>"
        backups = Path(self.tmp) / "_backups"
        editor = ag.AgenteEditorPropio()
        with (
            mock.patch.object(sc, "BACKUPS_DIR", backups),
            ...
            ok = editor._aplicar_modo_sobrescribir(
                "grande.py", "modifica funcion_003", self.grande, None, self.tmp, validar=False
            )
        self.assertTrue(ok)
    ...
    def test_respuesta_vacia_no_borra_el_archivo(self):
        ...
        with (
            mock.patch.object(sc, "BACKUPS_DIR", backups),
            mock.patch.object(sc, "_enviar_al_proveedor", return_value="sin marcadores"),
            mock.patch.object(sc, "MAX_CONTEXT_LINES", 30),
        ):
            ok = editor._aplicar_modo_sobrescribir(
                "grande.py", "modifica funcion_003", self.grande, None, self.tmp, validar=False
