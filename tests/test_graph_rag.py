...
class TestConstruccionGrafo(unittest.TestCase):
    ...
    def test_nodos_archivos(self):
        ...
        for rel in ("main.py", "animales.py", "servicios/pagos.py", "servicios/__init__.py"):
    ...
    def test_nodos_funciones_y_clases(self):
        ...
        self.assertEqual(grafo["nodos"]["animales.py::Animal"]["tipo"], "clase")
        self.assertEqual(grafo["nodos"]["main.py::Perro"]["tipo"], "clase")

    def test_arista_import(self):
        grafo = gr._extraer_nodos_y_aristas(self.dir)
        imports = {(a["origen"], a["destino"]) for a in grafo["aristas"] if a["tipo"] == "import"}
        self.assertIn(("main.py", "servicios/pagos.py"), imports)
        self.assertIn(("main.py", "animales.py"), imports)

    def test_arista_llamada(self):
        grafo = gr._extraer_nodos_y_aristas(self.dir)
        llamadas = {(a["origen"], a["destino"]) for a in grafo["aristas"] if a["tipo"] == "llamada"}
        self.assertIn(("main.py::cobrar", "servicios/pagos.py::procesar"), llamadas)

    def test_arista_herencia(self):
        grafo = gr._extraer_nodos_y_aristas(self.dir)
        herencias = {
            (a["origen"], a["destino"]) for a in grafo["aristas"] if a["tipo"] == "herencia"
        }
    ...
    def test_directorio_ignorado(self):
        ...
        (Path(self.dir) / "__pycache__" / "x.py").write_text("x = 1\n", encoding="utf-8")
        grafo = gr._extraer_nodos_y_aristas(self.dir)
...
class TestExpandirContexto(unittest.TestCase):
    ...
    def test_entrantes_primero(self):
        ...
        res = gr.expandir_contexto(["servicios/pagos.py"], self.grafo, notificar=False)
        self.assertEqual(res[0], "servicios/pagos.py")
        self.assertIn("main.py", res)

    def test_salientes(self):
        res = gr.expandir_contexto(["animales.py"], self.grafo, notificar=False)
        # animales.py no importa nada del proyecto, pero main.py lo usa.
        self.assertIn("main.py", res)

    def test_max_adicionales(self):
        res = gr.expandir_contexto(
            ["servicios/pagos.py", "animales.py"], self.grafo, max_adicionales=1, notificar=False
        )
        self.assertEqual(len(res), 3)  # 2 originales + 1

    def test_sin_grafo_o_vacio(self):
        self.assertEqual(gr.expandir_contexto(["a.py"], {}, notificar=False), ["a.py"])
        self.assertEqual(gr.expandir_contexto([], self.grafo), [])

    def test_no_duplica_y_mantiene_originales(self):
        res = gr.expandir_contexto(
            ["main.py", "servicios/pagos.py"], self.grafo, max_adicionales=5, notificar=False
        )
...
class TestCache(unittest.TestCase):
    ...
    def test_crea_y_reutiliza_cache(self):
        ...
        with mock.patch.object(
            gr, "_extraer_nodos_y_aristas", side_effect=AssertionError("no rebuild")
        ):
...
class TestIntegracionReactAgent(unittest.TestCase):
    ...
    def _agente(self, graph_rag):
        ...
        with mock.patch.object(ra.sc, "cargar_configuracion", return_value={}):
    ...
    def test_flag_on_expande(self):
        ...
        grafo = gr._extraer_nodos_y_aristas(self.dir)
        with mock.patch.object(agente, "_grafo_del_proyecto", return_value=grafo):
            res = agente._tool_buscar_codigo({"patron": "procesar"})
        self.assertTrue(res["ok"])
    ...
    def test_grafo_en_cache_tras_primera_construccion(self):
        agente = self._agente(True)
        with mock.patch.object(
            gr, "construir_grafo", return_value={"nodos": {}, "aristas": []}
        ) as c:
...
class TestFlagsYEntorno(unittest.TestCase):
    ...
    def test_env_activa(self):
        ...
        self.assertTrue(sc._graph_rag_activo(sc.crear_parser().parse_args(["consulta"])))
    ...
    def test_sin_flag_ni_env_desactivado(self):
        ...
        self.assertFalse(sc._graph_rag_activo(sc.crear_parser().parse_args(["consulta"])))
