...
class TestCondicionesPlan(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open(os.path.join(self.tmp, "main.py"), "w", encoding="utf-8") as fh:
            fh.write("def main():\n    pass\n")
    ...
    def test_archivo_existe_verdadero(self):
        self.assertTrue(sc._evaluar_condicion("archivo_existe('main.py')", self.tmp))

    def test_archivo_existe_falso(self):
        self.assertFalse(sc._evaluar_condicion('archivo_existe("no.py")', self.tmp))

    def test_archivo_contiene(self):
        self.assertTrue(sc._evaluar_condicion("archivo_contiene('main.py', 'def main')", self.tmp))
        self.assertFalse(
            sc._evaluar_condicion("archivo_contiene('main.py', 'clase_inexistente')", self.tmp)
        )
    ...
    def test_desconocida_devuelve_false_sin_excepcion(self):
        self.assertFalse(sc._evaluar_condicion("funcion_rara('x')", self.tmp))
...
class TestNormalizarDependencias(unittest.TestCase):
    ...
    def test_strings_numericos_y_duplicados(self):
        self.assertEqual(sc._normalizar_dependencias(["1", 1, "0"]), [0, 1])
...
class TestHerramientasMcpAvanzadas(unittest.TestCase):
    ...
    def test_registradas_en_predefinidas(self):
        for nombre in ("semantic_search", "ast_avanzado"):
            ...
            self.assertFalse(sc.HERRAMIENTAS_PREDEFINIDAS[nombre]["requiere_permiso"])
    ...
    def test_semantic_search_integrada_en_dispatcher(self):
        with mock.patch.object(
            sc, "_tool_semantic_search", return_value={"ok": True}
        ) as herramienta:
    ...
    def test_dispatcher_conoce_ast_avanzado(self):
        with mock.patch.object(sc, "_tool_ast_avanzado", return_value={"ok": True}) as herramienta:
            res = sc._ejecutar_herramienta_mcp("ast_avanzado", {"ruta": "x.py"}, confirmar=False)
            self.assertTrue(res["ok"])
...
class TestPlanParalelo(unittest.TestCase):
    ...
    def _pasos(self):
        return [
            ...
            {
                "descripcion": "depende del paso 1 (falla)",
                "accion": "ejecutar",
                "comando": "echo c",
                "dependencias": [1],
                "condicion": "",
            ...
            {
                "descripcion": "depende del paso 2 (éxito)",
                "accion": "ejecutar",
                "comando": "echo d",
                "dependencias": [2],
                "condicion": "",
    ...
    def test_dependencias_respetadas(self):
        ...
        args = _args_plan()
        with (
            mock.patch.object(sc, "_ejecutar_paso_plan", side_effect=falso_paso),
            mock.patch.object(sc, "_git_commit_paso"),
        ):
            resultados = sc._ejecutar_plan_en_paralelo(self._pasos(), args, ".", max_hilos=4)
    ...
    def test_condicion_salta_paso_en_paralelo(self):
        ...
        with mock.patch.object(sc, "_ejecutar_paso_plan") as ejecutado:
            resultados = sc._ejecutar_plan_en_paralelo(pasos, args, ".", max_hilos=2)
...
class TestFlagParalelo(unittest.TestCase):
    ...
    def test_valor_personalizado(self):
        args = sc.crear_parser().parse_args(["--plan", "tarea", "--paralelo", "4"])
...
class TestChatComandosGrafo(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open(os.path.join(self.tmp, "utils.py"), "w", encoding="utf-8") as fh:
            fh.write("# utilidades\n")
        os.chdir(self.tmp)
    ...
    def test_dependencias_directas_e_inversas(self):
        grafo = sc._grafo_dependencias(".")
        directas = [e["destino"] for e in grafo["enlaces"] if e["origen"] == "app.py"]
        inversas = [e["origen"] for e in grafo["enlaces"] if e["destino"] == "utils.py"]
