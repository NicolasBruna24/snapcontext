...
class TestFlagsCli310(unittest.TestCase):
    ...
    def test_flag_diagnostico(self):
        self.assertTrue(sc.crear_parser().parse_args(["--diagnostico"]).diagnostico)

    def test_flag_reparar(self):
        self.assertTrue(sc.crear_parser().parse_args(["--reparar"]).reparar)

    def test_flag_bienvenida(self):
        self.assertTrue(sc.crear_parser().parse_args(["--bienvenida"]).bienvenida)
...
class TestApiKeyDetection(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._patcher = mock.patch.object(sc, "cargar_configuracion", return_value={})
        self._patcher.start()
...
class TestEleccionModeloLigero(unittest.TestCase):
    ...
    def test_prefiere_llama32(self):
        self.assertEqual(sc._elegir_modelo_ligero(["llama3.1:70b", "llama3.2", "phi3"]), "llama3.2")

    def test_coincidencia_parcial(self):
        self.assertEqual(
            sc._elegir_modelo_ligero(["llama3.2:latest", "codellama"]), "llama3.2:latest"
        )
...
class TestEstadoOllama(unittest.TestCase):
    ...
    def test_ollama_no_instalado(self):
        with mock.patch.object(
            ...
            return_value=([], "No se encontró 'ollama' en el PATH. ¿Está instalado?"),
        ):
...
class TestProveedorOffline(unittest.TestCase):
    ...
    def _sin_guardado(self):
        ...
        p3 = mock.patch.object(sc, "_preguntar_guardar_config", return_value=False)
        return p1, p2, p3

    def test_sin_nada_lanza_mensaje_claro(self):
        p1, p2, p3 = self._sin_guardado()
        with p1, p2, p3, mock.patch.object(sc, "_proveedor_offline", return_value=None):
            with self.assertRaises(RuntimeError) as ctx:
                sc._determinar_proveedor(_args())
        self.assertIn("No se encontró una API key ni Ollama", str(ctx.exception))
    ...
    def test_con_ollama_usa_offline(self):
        ...
        with (
            ...
            p3,
            mock.patch.object(
                sc, "_proveedor_offline", return_value={"provider": "ollama", "model": "llama3.2"}
            ),
        ):
    ...
    def test_proveedor_offline_sin_modelos(self):
        with mock.patch.object(
            sc, "_estado_ollama", return_value={"instalado": False, "modelos": [], "error": "x"}
        ):
...
class TestDiagnostico(unittest.TestCase):
    ...
    def test_estado_memoria_inexistente(self):
        with mock.patch.object(sc, "DB_PATH", Path(os.devnull).parent / "no-existe.db"):
