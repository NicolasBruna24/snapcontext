"""Tests presentacion."""
import io, presentacion as pres
...
class TestPresentacion:
    ...
    def test_msgs(self, capsys):
        pres.info("i"); pres.exito("e"); pres.aviso("a"); pres.error("r")
    def test_depurar(self, capsys):
        pres.depurar("d"); assert capsys.readouterr().out == ""
    def test_cb(self):
        pres.fijar_evento_callback(None); assert pres.EVENTO_CALLBACK is None
