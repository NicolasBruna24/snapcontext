...
class BasePlugins(unittest.TestCase):
    ...
    def _crear_plugin(self, nombre="saludos", habilitado=True, requiere_permiso=False):
        ...
        (carpeta / "plugin.json").write_text(json.dumps(manifest), encoding="utf-8")
...
class TestManifest(BasePlugins):
    ...
    def test_manifest_sin_herramientas_invalido(self):
        ...
        carpeta.mkdir()
        (carpeta / "plugin.json").write_text(json.dumps({"nombre": "vacio"}), encoding="utf-8")
...
class TestRegistroMCP(BasePlugins):
    ...
    def test_ejecucion_por_subproceso(self):
        ...
        llamada = sc._ejecutar_herramienta_mcp("saludos_hola", {"nombre": "Ada"}, confirmar=False)
...
class TestInstalacion(BasePlugins):
    def _plugin_fuente(self):
        ...
        manifest = {
            ...
            "permisos": [],
            "herramientas": [
                {"nombre": "miplug_ping", "script": "ping.py", "requiere_permiso": False}
            ],
        }
        (carpeta / "plugin.json").write_text(json.dumps(manifest), encoding="utf-8")
    ...
    def test_instalar_desde_carpeta_local(self):
        codigo = sc._plugin_instalar(str(self._plugin_fuente()), auto=True)
    ...
    def test_instalacion_externa_cancelada_no_copia(self):
        with (
            ...
            self.assertEqual(sc._plugin_instalar("usuario/plugin"), 1)
    ...
    def test_create_y_update(self):
        ...
        self.assertTrue((Path(instalados["nuevoplug"]["ruta"]) / "saluda.py").is_file())
