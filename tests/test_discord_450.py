...
class BaseDiscord(unittest.TestCase):
    ...
    def _interaccion(comando="snap", consulta=None, tipo=2):
        ...
        if consulta is not None:
            datos["data"]["options"] = [{"name": "tarea", "type": 3, "value": consulta}]
        return datos
...
class TestConfiguracion(BaseDiscord):
    ...
    def test_valores_desde_config_json(self):
        config = {"discord": {"public_key": "pk-cfg", "application_id": "123", "bot_token": "tok"}}
...
class TestVerificacionFirma(BaseDiscord):
    def test_firma_valida_aceptada(self):
        ...
        privada = Ed25519PrivateKey.from_private_bytes(bytes.fromhex(_PRIVADA_HEX))
        cuerpo = b'{"type": 1}'
        timestamp = "1700000000"
        firma = privada.sign(timestamp.encode() + cuerpo).hex()
        with mock.patch.object(dg, "obtener_public_key", return_value=_PUBLICA_HEX):
...
class TestHandlerInteracciones(BaseDiscord):
    def test_ping_responde_pong(self):
        self.assertEqual(
            asyncio.run(dg.handle_discord_interaction(self._interaccion(tipo=1))), {"type": 1}
        )
    ...
    def test_comando_devuelve_respuesta_diferida(self):
        async def flujo():
            ...
            with (
                mock.patch.object(dg, "send_discord_message", side_effect=nada),
                mock.patch("telegram_gateway._ejecutar_pipeline", return_value="✅ listo"),
            ):
    ...
    def test_respuesta_se_envia_en_segundo_plano(self):
        ...
        async def flujo():
            with (
                mock.patch.object(dg, "send_discord_message", side_effect=fake_send),
                mock.patch("telegram_gateway._ejecutar_pipeline", return_value="✅ listo"),
            ):
                resultado = await dg.handle_discord_interaction(
                    self._interaccion("snap", "arregla login")
    ...
    def test_start_envia_bienvenida(self):
        ...
        async def flujo():
            with mock.patch.object(dg, "send_discord_message", side_effect=fake_send):
                resultado = await dg.handle_discord_interaction(self._interaccion("start"))
                await esperar_tareas(resultado)
    ...
    def test_extraer_consulta(self):
        ...
        self.assertEqual(
            (comando, consulta, extra), ("fix", "arreglar el checkout", ["--test-loop"])
        )
        comando, consulta, extra = dg._extraer_consulta(
            self._interaccion("plan", "migrar a pytest")
        )
        self.assertEqual((comando, consulta, extra), ("plan", "migrar a pytest", ["--plan"]))
        # /start y /help se reconocen por nombre (respuesta directa).
        self.assertEqual(dg._extraer_consulta(self._interaccion("start")), ("start", "", []))
        self.assertEqual(dg._extraer_consulta(self._interaccion("help")), ("help", "", []))
...
class TestEnvioMensajes(BaseDiscord):
    ...
    def test_followup_usa_webhooks_de_interaccion(self):
        ...
        with mock.patch.object(dg.httpx, "AsyncClient", return_value=cliente):
            ok = asyncio.run(
                dg.send_discord_message(None, "hola", interaction_id="999", interaction_token="TOK")
            )
        self.assertTrue(ok)
    ...
    def test_mensaje_largo_se_envia_como_archivo(self):
        ...
        with mock.patch.object(dg.httpx, "AsyncClient", return_value=cliente):
            ok = asyncio.run(
                dg.send_discord_message("https://hook", "x" * (dg.DISCORD_MAX_MENSAJE + 300))
            )
        self.assertTrue(ok)
    ...
    def test_error_de_red_no_lanza(self):
        ...
        cliente.__aexit__ = mock.AsyncMock(return_value=False)
        with mock.patch.object(dg.httpx, "AsyncClient", return_value=cliente):
            self.assertFalse(asyncio.run(dg.send_discord_message("https://hook", "x")))
...
class TestEndpointWebhook(unittest.TestCase):
    def test_endpoint_discord(self):
        ...
        # Sin public key → 503.
        with mock.patch.object(dg, "obtener_public_key", return_value=None):
            self.assertEqual(http.post("/webhook/discord", json={"type": 1}).status_code, 503)

        # Con public key pero sin firma válida → 401.
        with mock.patch.object(dg, "obtener_public_key", return_value=_PUBLICA_HEX):
            self.assertEqual(http.post("/webhook/discord", json={"type": 1}).status_code, 401)
        ...
        with (
            mock.patch.object(dg, "obtener_public_key", return_value=_PUBLICA_HEX),
            mock.patch.object(dg, "handle_discord_interaction", side_effect=nada),
        ):
            r = http.post(
                "/webhook/discord",
                content=cuerpo,
                headers={
                    "X-Signature-Ed25519": firma,
