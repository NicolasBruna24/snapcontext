...
class TestFlagPorDefecto(unittest.TestCase):
    ...
    def test_aider_sigue_disponible(self):
        args = sc.crear_parser().parse_args(["--editor", "aider", "consulta"])
...
class TestHeuristicaEstrategias(unittest.TestCase):
    ...
    def test_cadena_auto_estructural_incluye_ast_primero(self):
        with mock.patch.object(sc, "_ast_disponible", return_value=True):
            cadena = self.agente._cadena_modos(
                "modulo.py", "renombra la función procesar a manejar", "auto"
            )
    ...
    def test_cadena_sin_ast_empieza_por_parche(self):
        with mock.patch.object(sc, "_ast_disponible", return_value=False):
            cadena = self.agente._cadena_modos("app.js", "añadir validación al formulario", "auto")
    ...
    def test_ejecutar_intenta_estrategias_en_orden_y_falla_claro(self):
        ...
        try:
            llamadas = []
            with (
                mock.patch.object(sc, "_ast_disponible", return_value=True),
                mock.patch.object(
                    agente,
                    "editar_ast",
                    side_effect=lambda *a, **k: llamadas.append("ast") or False,
                ),
                mock.patch.object(
                    agente,
                    "_aplicar_modo_parche",
                    side_effect=lambda *a, **k: llamadas.append("parche") or False,
                ),
                mock.patch.object(
                    agente,
                    "_aplicar_modo_sobrescribir",
                    side_effect=lambda *a, **k: llamadas.append("sobrescribir") or False,
                ),
            ):
                ok = agente.ejecutar(["m.py"], "renombra foo a bar", directorio=tmp, auto=True)
            self.assertFalse(ok)
...
class TestConflictosYErrores(unittest.TestCase):
    ...
    def test_menu_conflicto_aplicar_de_todas_formas(self):
        with (
            mock.patch.object(sc, "_menu_conflicto_parche", return_value="a"),
            mock.patch.object(self.agente, "aplicar_parche", return_value=False),
            mock.patch.object(self.agente, "sobrescribir", return_value=True) as sob,
        ):
    ...
    def test_menu_conflicto_cancelar_conserva_original(self):
        with (
            mock.patch.object(sc, "_menu_conflicto_parche", return_value="c"),
            mock.patch.object(self.agente, "aplicar_parche", return_value=False) as aplicar,
        ):
            resultado = self.agente._aplicar_con_conflicto(
                "m.py", "--- diff ---", self.tmp, "x = 1\n"
            )
        ...
        self.assertEqual((Path(self.tmp) / "m.py").read_text(encoding="utf-8"), "x = 1\n")

    def test_auto_no_muestra_menu_y_devuelve_reintentar(self):
        with (
            mock.patch.object(sc, "_menu_conflicto_parche") as menu,
            mock.patch.object(self.agente, "aplicar_parche", return_value=False),
        ):
            resultado = self.agente._aplicar_con_conflicto(
                "m.py", "--- diff ---", self.tmp, "x = 1\n", auto=True
    ...
    def test_fallo_registra_en_logs(self):
        ...
        try:
            sc._registrar_fallo_editor(
                "m.py", "tarea imposible", ["parche", "sobrescribir"], "sin cambio válido"
            )
    ...
    def test_mensaje_error_contiene_sugerencia_aider(self):
        ...
        try:
            capturado = {}
            with (
                mock.patch.object(sc, "_ast_disponible", return_value=False),
                mock.patch.object(agente, "_aplicar_modo_parche", return_value=False),
                mock.patch.object(agente, "_aplicar_modo_sobrescribir", return_value=False),
                mock.patch.object(
                    sc, "error", side_effect=lambda t: capturado.setdefault("texto", t)
                ),
            ):
