...
class TestAutenticacion(BaseAPI):
    ...
    def test_clave_invalida_rechazada(self):
        r = self.client.get(f"{API_PREFIJO}/skills", headers={"X-API-Key": "incorrecta"})
...
class TestTareasAsincronas(BaseAPI):
    ...
    def test_query_responde_202_y_completa(self):
        with mock.patch.object(sc, "flujo_principal", return_value=0) as flu:
            r = self.client.post(
                f"{API_PREFIJO}/query", json={"consulta": "revisar login"}, headers=HEADERS
            )
    ...
    def test_plan_fallido_marca_tarea_como_fallida(self):
        with mock.patch.object(sc, "_ejecutar_planificador", return_value=1):
            r = self.client.post(
                f"{API_PREFIJO}/plan", json={"consulta": "planear algo"}, headers=HEADERS
            )
            self.assertEqual(r.status_code, 202)
    ...
    def test_error_en_tarea_se_reporta(self):
        with mock.patch.object(sc, "flujo_principal", side_effect=RuntimeError("boom")):
            r = self.client.post(f"{API_PREFIJO}/query", json={"consulta": "x"}, headers=HEADERS)
            estado = self._esperar_estado(r.json()["task_id"])
    ...
    def _esperar_estado(self, task_id: str, timeout: float = 5.0) -> dict:
        ...
        while time.monotonic() < limite:
            r = self.client.get(f"{API_PREFIJO}/tasks/{task_id}", headers=HEADERS)
            if r.status_code == 200 and r.json()["estado"] not in ("pendiente", "ejecutando"):
                return r.json()
            time.sleep(0.02)
...
class TestChatYSkills(BaseAPI):
    ...
    def test_chat_devuelve_respuesta_del_proveedor(self):
        with (
            mock.patch.object(sc, "_enviar_al_proveedor", return_value="Â¡Hola!") as enviar,
            mock.patch.object(sc, "cargar_configuracion", return_value={"provider": "groq"}),
        ):
            r = self.client.post(
                f"{API_PREFIJO}/chat",
                json={"mensaje": "hola", "historial": [{"role": "user", "content": "previo"}]},
                headers=HEADERS,
            )
        self.assertEqual(r.status_code, 200)
    ...
    def test_chat_error_de_proveedor_es_502(self):
        with (
            mock.patch.object(sc, "_enviar_al_proveedor", side_effect=RuntimeError("sin clave")),
            mock.patch.object(sc, "cargar_configuracion", return_value={}),
        ):
            r = self.client.post(f"{API_PREFIJO}/chat", json={"mensaje": "x"}, headers=HEADERS)
        self.assertEqual(r.status_code, 502)

    def test_skills_lista(self):
        with mock.patch.object(
            sc, "_skill_listar", return_value=[{"nombre": "modo-oscuro"}]
        ) as lst:
            r = self.client.get(f"{API_PREFIJO}/skills?archivados=true", headers=HEADERS)
        self.assertEqual(r.status_code, 200)
        datos = r.json()
...
class TestDaemonYFlags(BaseAPI):
    def test_daemon_estado_por_defecto_inactivo(self):
        ...
        try:
            r = self.client.post(
                f"{API_PREFIJO}/daemon", json={"accion": "estado"}, headers=HEADERS
            )
            self.assertEqual(r.status_code, 200)
    ...
    def test_daemon_accion_invalida_es_400(self):
        r = self.client.post(f"{API_PREFIJO}/daemon", json={"accion": "volar"}, headers=HEADERS)
        self.assertEqual(r.status_code, 400)
