...
class TestConfiguracion(unittest.TestCase):
    ...
    def test_token_desde_config_json(self):
        with mock.patch.object(
            tg, "_ruta_config", return_value=self._cfg({"telegram": {"bot_token": "tok-cfg"}})
        ):
    ...
    def test_sin_configuracion_devuelve_none(self):
        with mock.patch.object(tg, "_ruta_config", return_value=self._cfg({})):
    ...
    def test_guardar_configuracion(self):
        ...
        with mock.patch.object(tg, "_ruta_config", return_value=destino):
            seccion = tg.guardar_configuracion_telegram("123:ABC", "https://x.ngrok.io")
        guardado = json.loads(destino.read_text(encoding="utf-8"))
        self.assertEqual(guardado["telegram"]["bot_token"], "123:ABC")
        self.assertEqual(guardado["telegram"]["webhook_url"], "https://x.ngrok.io")
...
class TestEnvioMensajes(unittest.TestCase):
    def test_send_message_ok(self):
        ...
        with (
            mock.patch.object(tg, "obtener_token", return_value="T"),
            mock.patch.object(tg.httpx, "AsyncClient", return_value=cliente) as fake_cli,
        ):
            ok = asyncio.run(tg.send_telegram_message(42, "hola *mundo*"))
        self.assertTrue(ok)
    ...
    def test_mensaje_largo_se_envia_como_documento(self):
        ...
        with (
            mock.patch.object(tg, "obtener_token", return_value="T"),
            mock.patch.object(tg.httpx, "AsyncClient", return_value=cliente),
        ):
...
class TestHandlerUpdates(unittest.TestCase):
    def _update(self, texto, chat_id=42):
        return {
            "update_id": 1,
            "message": {"message_id": 1, "chat": {"id": chat_id}, "text": texto},
        }
    ...
    def test_start_responde_bienvenida(self):
        ...
        with mock.patch.object(tg, "send_telegram_message", side_effect=fake_send):
    ...
    def test_mensaje_lanza_tarea_en_segundo_plano(self):
        ...
        async def flujo():
            with (
                mock.patch.object(tg, "send_telegram_message", side_effect=fake_send),
                mock.patch.object(tg, "run_agent_async", return_value="✅ listo"),
            ):
...
class TestEndpointWebhook(unittest.TestCase):
    def test_webhook_endpoint(self):
        ...
        with mock.patch.object(tgmod, "obtener_token", return_value=None):
            r = cliente_http.post("/webhook/telegram", json=self._update("/start"))
        ...
        with (
            mock.patch.object(tgmod, "obtener_token", return_value="T"),
            mock.patch.object(tgmod, "handle_telegram_update", side_effect=nada),
        ):
            r = cliente_http.post("/webhook/telegram", json=self._update("/start"))
