...
def _plan():
    ...
    return {
        ...
        "pasos": [{"descripcion": "crear módulo", "accion": "editar", "archivos": ["src/auth.py"]}],
        "dependencias": [],
    }
...
class TestArquitecto(BaseMulti):
    ...
    def test_genera_plan_json(self):
        plan_json = (
            '{"objetivo": "x", "modulos": ["m"], "archivos": ["a.py"],'
            ' "pasos": [], "dependencias": []}'
        )
        with mock.patch.object(sc, "_enviar_al_proveedor", return_value=plan_json):
    ...
    def test_plan_invalido_degenera(self):
        with mock.patch.object(sc, "_enviar_al_proveedor", return_value="no es json"):
            arqu = ma.Arquitecto(proveedor="ollama")
    ...
    def test_publica_plan_en_buzon(self):
        buz = ma.Buzon()
        plan_json = (
            '{"objetivo": "x", "archivos": [], "pasos": [], "modulos": [], "dependencias": []}'
        )
        with mock.patch.object(sc, "_enviar_al_proveedor", return_value=plan_json):
...
class TestProgramador(BaseMulti):
    ...
    def test_implementa_ok(self):
        ...
        with mock.patch.object(ag.AgenteEditorPropio, "ejecutar", return_value=True):
    ...
    def test_implementa_falla(self):
        prog = self._programador()
        with mock.patch.object(ag.AgenteEditorPropio, "ejecutar", return_value=False):
            res = prog.implementar("t", _plan(), ["src/auth.py"], self.dir)
        self.assertFalse(res["ok"])

    def test_plan_sin_archivos_noop(self):
        prog = self._programador()
        res = prog.implementar("t", {"archivos": [], "objetivo": "x"}, [], self.dir)
        self.assertTrue(res["ok"])
    ...
    def test_usa_archivos_del_plan_si_no_se_pasan(self):
        prog = self._programador()
        with mock.patch.object(ag.AgenteEditorPropio, "ejecutar", return_value=True) as ejec:
...
class TestTester(BaseMulti):
    ...
    def test_ejecuta_pruebas_ok(self):
        tester = ma.Tester()
        with (
            mock.patch.object(det, "resolver_comando_test", return_value="pytest"),
            mock.patch.object(sc, "_ejecutar_comando", return_value=(0, "OK", "")),
        ):
    ...
    def test_pruebas_fallan(self):
        ...
        with (
            mock.patch.object(det, "resolver_comando_test", return_value="pytest"),
            mock.patch.object(sc, "_ejecutar_comando", return_value=(1, "", "boom")),
        ):
            res = tester.ejecutar(self.dir)
        self.assertFalse(res["ok"])
    ...
    def test_sin_comando_detectado(self):
        tester = ma.Tester()
        with mock.patch.object(det, "resolver_comando_test", return_value=None):
            res = tester.ejecutar(self.dir)
        self.assertFalse(res["ok"])
    ...
    def test_comando_explicito_gana(self):
        tester = ma.Tester()
        with (
            mock.patch.object(det, "resolver_comando_test", return_value="pytest") as resolver,
            mock.patch.object(sc, "_ejecutar_comando", return_value=(0, "", "")) as ejec,
        ):
    ...
    def test_env_comando_test(self):
        ...
        tester = ma.Tester()
        with (
            mock.patch.object(det, "resolver_comando_test", return_value=None) as resolver,
...
class TestSupervisor(BaseMulti):
    ...
    def test_flujo_completo_ok(self):
        sup = self._sup()
        with (
            mock.patch.object(ma.Arquitecto, "generar_plan", return_value=_plan()),
            mock.patch.object(ma.Programador, "implementar", return_value={"ok": True}),
            mock.patch.object(
                ma.Tester, "ejecutar", return_value={"ok": True, "detectado": True, "codigo": 0}
            ),
        ):
            res = sup.ejecutar()
        self.assertTrue(res["ok"])
    ...
    def test_realimentacion_hasta_verde(self):
        ...
        with (
            mock.patch.object(ma.Arquitecto, "generar_plan", return_value=_plan()),
            mock.patch.object(ma.Programador, "implementar", return_value={"ok": True}) as imp,
            mock.patch.object(ma.Tester, "ejecutar", side_effect=lambda *a, **k: next(pruebas)),
        ):
            res = sup.ejecutar()
        self.assertTrue(res["ok"])
    ...
    def test_sin_pruebas_se_da_por_terminado(self):
        ...
        with (
            ...
            mock.patch.object(ma.Programador, "implementar", return_value={"ok": True}),
            mock.patch.object(
                ma.Tester, "ejecutar", return_value={"ok": False, "detectado": False, "codigo": -1}
            ),
        ):
            res = sup.ejecutar()
        self.assertTrue(res["ok"])
    ...
    def test_agota_reintentos(self):
        ...
        with (
            ...
            mock.patch.object(
                ma.Tester,
                "ejecutar",
                return_value={"ok": False, "detectado": True, "codigo": 1, "stderr": "x"},
            ),
        ):
            res = sup.ejecutar()
        self.assertFalse(res["ok"])
    ...
    def test_arquitecto_falla_aborta(self):
        ...
        with mock.patch.object(
            ma.Arquitecto, "generar_plan", side_effect=RuntimeError("boom de llm")
        ):
            res = sup.ejecutar()
        self.assertFalse(res["ok"])
    ...
    def test_programador_falla_no_llama_tester(self):
        sup = self._sup()
        with (
            mock.patch.object(ma.Arquitecto, "generar_plan", return_value=_plan()) as arq,
            mock.patch.object(
                ma.Programador, "implementar", return_value={"ok": False, "error": "no edit"}
            ),
        ):
            res = sup.ejecutar()
        self.assertFalse(res["ok"])
    ...
    def test_interactivo_cancelacion(self):
        ...
        with (
            mock.patch.object(ma.Arquitecto, "generar_plan", return_value=_plan()),
            mock.patch.object(ma.Supervisor, "_confirmar_plan", return_value=False),
        ):
            res = sup.ejecutar()
        self.assertFalse(res["ok"])
    ...
    def test_auto_no_pregunta(self):
        ...
        with (
            ...
            mock.patch("ui.preguntar_interactivo", side_effect=AssertionError("no debe preguntar")),
        ):
            res = sup.ejecutar()
        self.assertTrue(res["ok"])
