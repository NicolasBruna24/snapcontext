...
class TestEnrutarTarea(unittest.TestCase):
    ...
    def test_enrutado_con_config(self):
        config = {
            "model_routing": {"edicion_critica": {"provider": "gemini", "model": "gemini-2.5-pro"}}
        }
...
class TestEsTareaCompleja(unittest.TestCase):
    ...
    def test_umbral_configurable(self):
        config = {"model_routing": {"umbral_complejidad": {"longitud_consulta": 5}}}
        self.assertTrue(mr.es_tarea_compleja("una dos tres cuatro cinco seis", config=config))

    def test_umbral_invalido_usa_defecto(self):
        config = {"model_routing": {"umbral_complejidad": {"longitud_consulta": "mucho"}}}
        self.assertFalse(mr.es_tarea_compleja("hola qué tal", config=config))

    def test_comando_complejo_deploy(self):
        self.assertTrue(mr.es_tarea_compleja("haz deploy del servicio a producción"))

    def test_comando_complejo_docker(self):
        self.assertTrue(mr.es_tarea_compleja("ejecuta docker compose up -d y revisa los logs"))

    def test_consulta_normal_no_es_compleja(self):
        self.assertFalse(mr.es_tarea_compleja("añade un campo email al formulario de registro"))
    ...
    def test_archivo_grande_en_disco(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(mr.es_tarea_compleja("revisa esto", {"archivos": [grande]}))

    def test_archivo_pequeno_no_es_compleja(self):
        with tempfile.TemporaryDirectory() as tmp:
            peque = os.path.join(tmp, "peque.py")
            Path(peque).write_text("x = 1\n", encoding="utf-8")
            self.assertFalse(mr.es_tarea_compleja("revisa esto", {"archivos": [peque]}))

    def test_tamano_archivo_configurable(self):
        with tempfile.TemporaryDirectory() as tmp:
            ruta = os.path.join(tmp, "medio.py")
            Path(ruta).write_text("x = 1\n" * 15, encoding="utf-8")
            config = {"model_routing": {"umbral_complejidad": {"tamano_archivo": 10}}}
            self.assertTrue(mr.es_tarea_compleja("revisa", {"archivos": [ruta]}, config))

    def test_archivo_inexistente_no_rompe(self):
        self.assertFalse(mr.es_tarea_compleja("revisa esto", {"archivos": ["no_existe_12345.py"]}))
    ...
    def test_hint_lineas_sin_disco(self):
        ...
        self.assertFalse(mr.es_tarea_compleja("revisa", {"lineas_por_archivo": {"a.py": 50}}))
...
class TestObtenerOrdenPrioridad(unittest.TestCase):
    ...
    def test_entrada_como_dicts(self):
        config = {
            "model_routing": {"prioridad_local": [{"provider": "ollama", "model": "qwen3.5:9b"}]}
        }
        self.assertEqual(mr.obtener_orden_prioridad(False, config), [("ollama", "qwen3.5:9b")])
...
class TestSeleccionarModeloConFallback(unittest.TestCase):
    ...
    def test_simple_devuelve_local(self):
        prov, mod = mr.seleccionar_modelo_con_fallback("indexacion", _CFG_HIBRIDO, compleja=False)
    ...
    def test_sin_prioridades_delega_en_seleccionar_modelo(self):
        ...
        prov, mod = mr.seleccionar_modelo_con_fallback("edicion_critica", config, compleja=False)
        self.assertEqual((prov, mod), ("gemini", "gemini-2.5-pro"))

    def test_sin_nada_devuelve_none(self):
        prov, mod = mr.seleccionar_modelo_con_fallback("chat_general", {}, compleja=True)
        self.assertIsNone(prov)
        self.assertIsNone(mod)

    def test_solo_prioridad_local(self):
        config = {"model_routing": {"prioridad_local": ["ollama/llama3.2"]}}
        prov, mod = mr.seleccionar_modelo_con_fallback("edicion_critica", config, compleja=True)
...
class TestEnrutarTareaHibrido(unittest.TestCase):
    ...
    def test_consulta_larga_marca_compleja(self):
        resultado = mr.enrutar_tarea(" ".join(["palabra"] * 120), config=_CFG_HIBRIDO)
...
class TestFallbackEnvio(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sc = sc
        self._estado = (sc._MODEL_ROUTING_ACTIVO, sc._MODELO_EXPLICITO, sc._MODEL_FALLBACK_ACTIVO)
    ...
    def tearDown(self):
        (
            ...
            self.sc._MODEL_FALLBACK_ACTIVO,
        ) = self._estado
    ...
    def test_fallback_tras_error_de_api(self):
        ...
        with (
            mock.patch.object(
                sc,
                "_cargar_configuracion_routing",
                return_value=dict(_CFG_HIBRIDO["model_routing"]),
            ),
            mock.patch.object(
                sc,
                "_enviar_al_proveedor_unico",
                side_effect=[RuntimeError("connection timeout"), "respuesta"],
            ) as enviar,
        ):
        ...
        self.assertEqual(
            [c.args[:2] for c in enviar.call_args_list],
            [("ollama", "qwen3.5:9b"), ("ollama", "llama3.2")],
        )

    def test_error_autenticacion_no_reintenta(self):
        sc = self.sc
        error_clave = RuntimeError("No se encontró la variable de entorno GEMINI_API_KEY.")
        with (
            mock.patch.object(
                sc,
                "_cargar_configuracion_routing",
            ...
            mock.patch.object(sc, "_enviar_al_proveedor_unico", side_effect=error_clave) as enviar,
        ):
    ...
    def test_todos_fallan_error_claro(self):
        ...
        with (
            ...
            mock.patch.object(
                sc, "_enviar_al_proveedor_unico", side_effect=RuntimeError("boom")
            ) as enviar,
        ):
    ...
    def test_tarea_compleja_usa_nube_primero(self):
        ...
        with (
            ...
            mock.patch.object(sc, "_enviar_al_proveedor_unico", return_value="ok") as enviar,
        ):
            sc._enviar_al_proveedor(
                "ollama",
                "llama3.2",
    ...
    def test_mensaje_tarea_simple_local(self):
        ...
        with (
            ...
            mock.patch.object(sc, "_enviar_al_proveedor_unico", return_value="ok"),
        ):
            sc._enviar_al_proveedor(
                "ollama",
                "llama3.2",
        ...
        textos = " ".join(str(c.args[0]) for c in info.call_args_list if c.args)
        self.assertIn("🧠 Tarea simple. Usando modelo local: ollama/qwen3.5:9b", textos)
    ...
    def test_mensaje_tarea_compleja_cloud(self):
        ...
        textos = " ".join(str(c.args[0]) for c in info.call_args_list if c.args)
        self.assertIn(
            "🧠 Tarea compleja detectada. Usando modelo cloud: gemini/gemini-2.5-pro", textos
        )
    ...
    def test_mensaje_aviso_reintento(self):
        ...
        with (
            ...
            mock.patch.object(
                sc, "_enviar_al_proveedor_unico", side_effect=[RuntimeError("timeout"), "ok"]
            ),
        ):
            sc._enviar_al_proveedor(
                "ollama",
        ...
        textos = " ".join(str(c.args[0]) for c in aviso.call_args_list if c.args)
        self.assertIn("⚠️ Fallo en ollama/qwen3.5:9b. Reintentando con ollama/llama3.2", textos)

    def test_sin_prioridades_comportamiento_v624(self):
        sc = self.sc
        config = {"chat_general": {"provider": "gemini", "model": "gemini-2.5-pro"}}
        with (
            mock.patch.object(sc, "_cargar_configuracion_routing", return_value=config),
            mock.patch.object(sc, "_enviar_al_proveedor_unico", return_value="ok") as enviar,
        ):
            sc._enviar_al_proveedor(
                "ollama",
...
class TestFlagsCLI(unittest.TestCase):
    ...
    def test_no_model_fallback(self):
        ...
        args = sc.crear_parser().parse_args(["consulta", "--no-model-fallback"])
    ...
    def test_complejidad_umbral(self):
        ...
        args = sc.crear_parser().parse_args(["consulta", "--complejidad-umbral", "25"])
    ...
    def test_model_prioridad_local(self):
        ...
        args = sc.crear_parser().parse_args(
            ["consulta", "--model-prioridad-local", "ollama/qwen3.5:9b", "ollama/llama3.2"]
        )
        self.assertEqual(args.model_prioridad_local, ["ollama/qwen3.5:9b", "ollama/llama3.2"])
    ...
    def test_model_prioridad_nube(self):
        ...
        self.assertEqual(args.model_prioridad_nube, ["gemini/gemini-2.5-pro"])
    ...
    def test_configuracion_routing_efectiva_umbral(self):
        ...
        try:
            ...
            self.assertEqual(seccion["umbral_complejidad"]["longitud_consulta"], 25)
        finally:
            sc._configurar_hibrido_cli()
    ...
    def test_configuracion_routing_efectiva_prioridades(self):
        ...
        sc._configurar_hibrido_cli(
            prioridad_local=["ollama/llama3.2"], prioridad_nube=["deepseek/deepseek-v3"]
        )
        try:
            seccion = sc._configuracion_routing_efectiva()
            self.assertEqual(seccion["prioridad_local"], ["ollama/llama3.2"])
            self.assertEqual(seccion["prioridad_nube"], ["deepseek/deepseek-v3"])
        finally:
            sc._configurar_hibrido_cli()
    ...
    def test_existen_configuradores_hibrido(self):
        ...
        self.assertTrue(callable(getattr(sc, "_configurar_model_fallback", None)))
        self.assertTrue(callable(getattr(sc, "_configuracion_routing_efectiva", None)))
        self.assertTrue(callable(getattr(sc, "_es_error_autenticacion", None)))
        self.assertTrue(callable(getattr(sc, "_extraer_consulta_mensajes", None)))
...
class TestEsErrorAutenticacion(unittest.TestCase):
    ...
    def test_api_key_faltante(self):
        ...
        self.assertTrue(
            sc._es_error_autenticacion(
                RuntimeError("No se encontró la variable de entorno GEMINI_API_KEY.")
            )
        )
    ...
    def test_invalid_api_key(self):
        ...
        self.assertTrue(
            sc._es_error_autenticacion(RuntimeError("invalid_api_key: Incorrect API key provided"))
        )

    def test_error_api_normal_no_es_auth(self):
        import snapcontext as sc

        self.assertFalse(sc._es_error_autenticacion(RuntimeError("Connection timeout after 120s")))
...
class TestVersion630(unittest.TestCase):
    ...
    def test_version_6_30_0(self):
        ...
        self.assertEqual(sc.VERSION, "6.34.3")
