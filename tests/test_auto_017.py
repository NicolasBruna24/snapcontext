...
class TestPermisoRecordado(unittest.TestCase):
    def setUp(self):
        ...
        for attr, nombre in (("CONFIG_DIR", "d"), ("PERMISOS_PATH", "permisos.json")):
...
class TestPasoPlanAuto(unittest.TestCase):
    ...
    def test_auto_permite_sin_preguntar_si_no_hay_preferencia(self):
        ...
        with mock.patch.object(sc, "_confirmar_accion") as conf:
            ok, _ = sc._ejecutar_paso_plan(
                {"accion": "ejecutar", "descripcion": "eco", "comando": comando},
                args,
                str(self.dir_tmp),
            )
    ...
    def test_auto_respeta_nunca_guardado(self):
        ...
        with (
            ...
            ok, detalle = sc._ejecutar_paso_plan(
                {"accion": "ejecutar", "descripcion": "prohibido", "comando": "cmd /c echo x"},
                args,
                str(self.dir_tmp),
            )
    ...
    def test_auto_respeta_siempre_guardado(self):
        ...
        with mock.patch.object(sc, "_confirmar_accion") as conf:
            ok, _ = sc._ejecutar_paso_plan(
                {"accion": "ejecutar", "descripcion": "permitido", "comando": comando},
                args,
                str(self.dir_tmp),
            )
        conf.assert_not_called()
...
class TestPlanificadorAuto(unittest.TestCase):
    ...
    def test_auto_salta_confirmaciones_y_menues(self):
        ...
        with (
            mock.patch.object(sc, "_generar_plan", return_value=self.PASOS),
            mock.patch.object(
                sc, "_preguntar_si", side_effect=AssertionError("no debía preguntar")
            ),
            mock.patch.object(sc, "_ejecutar_paso_plan", return_value=(True, "ok")) as ep,
    ...
    def test_reintentos_automaticos_hasta_exito(self):
        ...
        efectos = [(False, "fallo1"), (False, "fallo2"), (True, "ok"), (True, "ok")]
        with (
            mock.patch.object(sc, "_generar_plan", return_value=self.PASOS),
            mock.patch.object(sc, "_ejecutar_paso_plan", side_effect=efectos) as ep,
        ):
            codigo = sc._ejecutar_planificador(_args_base(auto=True))
        self.assertEqual(codigo, 0)
    ...
    def test_sin_auto_se_mantiene_el_comportamiento_interactivo(self):
        ...
        with (
            mock.patch.object(sc, "_generar_plan", return_value=[self.PASOS[0]]),
            mock.patch.object(sc, "_preguntar_si", return_value=True) as ps,
            mock.patch("builtins.input", side_effect=["r", "r", "c"]),
            mock.patch.object(
                sc, "_ejecutar_paso_plan", side_effect=[(True, "ok"), (True, "ok2"), (True, "ok3")]
            ) as ep,
        ):
