...
class TestClasificacionTareas(unittest.TestCase):
    ...
    def test_renombrar(self):
        self.assertEqual(
            sc._editor_clasificar_tarea("renombrar la función calcular_total"), "renombrar"
        )

    def test_import(self):
        self.assertEqual(sc._editor_clasificar_tarea("añadir import de typing"), "añadir_import")

    def test_refactorizar(self):
        self.assertEqual(
            sc._editor_clasificar_tarea("refactorizar la clase Carrito"), "refactorizar_clase"
        )

    def test_funcion_nueva(self):
        self.assertEqual(
            sc._editor_clasificar_tarea("crear una función de validación"), "añadir_funcion"
        )

    def test_corregir_error(self):
        self.assertEqual(
            sc._editor_clasificar_tarea("corrige el bug del checkout"), "corregir_error"
        )

    def test_general(self):
        self.assertEqual(sc._editor_clasificar_tarea("mejorar rendimiento"), "general")
...
class TestDeteccionLenguaje(unittest.TestCase):
    ...
    def test_por_contenido_shebang(self):
        self.assertEqual(
            sc._detectar_lenguaje_contenido("#!/usr/bin/env python3\nprint(1)\n"), "python"
        )
        self.assertEqual(sc._detectar_lenguaje_contenido("#!/bin/bash\necho hola\n"), "bash")
...
class TestRutaYValidacionParche(unittest.TestCase):
    ...
    def test_validacion_detecta_cambio_concurrente(self):
        ...
        ok, detalle = sc._validar_parche_previo(self.PARCH, self.tmp, "viejo\n")
        self.assertFalse(ok)
...
class TestParsearHunks(unittest.TestCase):
    def test_hunks_basicos(self):
        ...
        self.assertEqual(cambios, [(" ", "ctx"), ("-", "old"), ("+", "new"), (" ", "ctx2")])
...
class TestAplicacionIncremental(unittest.TestCase):
    ...
    def test_aplica_con_desfase_de_lineas(self):
        ...
        original = "linea 1\nlinea 2\nlinea 3\nlinea 4\nlinea 5\n"
        nuevo = "linea 1\nlinea 2\nlinea 3\nlinea CUATRO\nlinea 5\n"
        nombre = self._escribir("g.py", "extra A\nextra B\n" + original)
        parche = sc._generar_parche(original, nuevo, nombre)
        self.assertTrue(sc._aplicar_hunks_incremental(parche, self.tmp))
        resultado = (Path(self.tmp) / nombre).read_text(encoding="utf-8")
        self.assertEqual(
...
class TestResolucionConConflictos(unittest.TestCase):
    ...
    def test_git_apply_exitoso_no_requiere_resolucion(self):
        with (
            mock.patch.object(sc, "_aplicar_parche", return_value=True),
            mock.patch.object(sc, "_aplicar_hunks_incremental") as inc,
        ):
            self.assertTrue(sc._aplicar_parche_con_resolucion(self.parche, self.tmp))
            inc.assert_not_called()

    def test_conflicto_cae_a_resolucion_incremental(self):
        with (
            mock.patch.object(sc, "_aplicar_parche", return_value=False),
            mock.patch.object(sc, "_aplicar_hunks_incremental", return_value=True) as inc,
        ):
            self.assertTrue(sc._aplicar_parche_con_resolucion(self.parche, self.tmp))
            inc.assert_called_once_with(self.parche, self.tmp, mostrar_diff=False)
    ...
    def test_validacion_previa_avisa_pero_intenta(self):
        ...
        with (
            ...
            self.assertTrue(ok)
            self.assertTrue(
                any("Validación previa fallida" in str(c.args[0]) for c in avisos.call_args_list)
            )
...
class TestSkillsEditor(unittest.TestCase):
    ...
    def test_guardar_skill_editor_usa_nombre_idempotente(self):
        with mock.patch.object(sc, "_skill_guardar", return_value=42) as guardar:
            sid = sc._skill_editor_guardar(
                "renombrar la función x", "modulo.py", "renombrar", estrategia="ast"
            )
    ...
    def test_guardar_skill_nunca_lanza(self):
        with mock.patch.object(sc, "_skill_guardar", side_effect=RuntimeError("db rota")):
            self.assertIsNone(sc._skill_editor_guardar("t", "a.py", "general"))

    def test_estrategia_desde_skill_valido(self):
        skill = {
            "id": 7,
            "nombre": "editor-renombrar",
            "confiabilidad": 0.8,
            "pasos": [{"accion": "editor_propio", "estrategia": "parche"}],
        }
        with mock.patch.object(sc, "_skill_buscar", return_value=skill):
            self.assertEqual(sc._skill_editor_estrategia("renombrar la función y"), "parche")
...
class TestIntegracionSkillsEnEjecutar(unittest.TestCase):
    ...
    def test_exito_guarda_patron_y_refuerza(self):
        ...
        with (
            mock.patch.object(sc, "_enviar_al_proveedor", return_value=diff),
            mock.patch.object(agente, "aplicar_parche", return_value=True),
            mock.patch.object(sc, "_skill_editor_estrategia", return_value=None) as buscar,
            mock.patch.object(sc, "_skill_editor_guardar", return_value=9) as guardar,
            mock.patch.object(sc, "_skill_registrar_exito", return_value=0.65) as reforzar,
        ):
            ok = agente.ejecutar(
                [archivo], "corrige el retorno", directorio=self.tmp, modo_edicion="parche"
            )
            self.assertTrue(ok)
    ...
    def test_skill_aprendido_prioriza_estrategia(self):
        ...
        with (
            mock.patch.object(agente, "_cadena_modos", return_value=["parche", "sobrescribir"]),
            mock.patch.object(sc, "_skill_editor_estrategia", return_value="sobrescribir"),
            mock.patch.object(sc, "_skill_editor_guardar", return_value=None),
            mock.patch.object(agente, "_aplicar_modo_sobrescribir", return_value=True) as sob,
            mock.patch.object(agente, "_aplicar_modo_parche") as parche,
        ):
            ok = agente.ejecutar([archivo], "cambia algo", directorio=self.tmp, modo_edicion="auto")
            self.assertTrue(ok)
...
class TestPromptEditacionEnriquecido(unittest.TestCase):
    ...
    def test_prompt_parche_contiene_lenguaje_e_instrucciones(self):
        ...
        with (
            mock.patch.object(sc, "_enviar_al_proveedor", return_value=diff) as enviar,
            mock.patch.object(agente, "aplicar_parche", return_value=True),
        ):
