...
class TestValidarSintaxis(BaseValidacion):
    ...
    def test_comando_no_disponible_omite(self):
        with (
            mock.patch.object(sc, "_lenguaje_archivo", return_value="dart"),
    ...
    def test_dart_fallback_analyze_indisponible(self):
        with (
            mock.patch.object(sc, "_lenguaje_archivo", return_value="dart"),
            mock.patch(
                "snapcontext.shutil.which", side_effect=lambda b: b if b == "dart" else None
            ),
            mock.patch(
                "snapcontext.subprocess.run", side_effect=[FileNotFoundError, _proc(0)]
            ) as run,
        ):
            ok, mensaje = sc._validar_sintaxis("lib/main.dart", "dart", self.tmp)
        self.assertTrue(ok)
        self.assertEqual(run.call_count, 2)
        self.assertEqual(run.call_args_list[1][0][0][0:2], ["dart", "format"])
    ...
    def test_java_y_node_comandos(self):
        with (
            mock.patch.object(sc, "_lenguaje_archivo", return_value="java"),
        ...
        with (
            mock.patch.object(sc, "_lenguaje_archivo", return_value="javascript"),
...
class TestFlagsValidacion(unittest.TestCase):
    ...
    def test_max_intentos_validacion_defecto_y_personalizado(self):
        ...
        args2 = sc.crear_parser().parse_args(["consulta", "--max-intentos-validacion", "5"])
...
class TestReintentosSobrescribir(BaseValidacion):
    ...
    def test_falla_y_luego_aprueba(self):
        ...
        with (
            ...
            mock.patch.object(
                sc, "_enviar_al_proveedor", return_value="def f(): return 2\n"
            ) as enviar,
            mock.patch.object(
                sc, "_validar_sintaxis", side_effect=[(False, "SyntaxError"), (True, "")]
            ) as val,
    ...
    def test_agota_intentos_cancela_sin_guardar(self):
        ...
        with (
            mock.patch.object(sc, "cargar_configuracion", return_value={}),
            mock.patch.object(sc, "_enviar_al_proveedor", return_value="malo()(") as enviar,
            mock.patch.object(sc, "_validar_sintaxis", return_value=(False, "SyntaxError")) as val,
    ...
    def test_validar_false_no_valida(self):
        ...
        with (
            mock.patch.object(sc, "cargar_configuracion", return_value={}),
            mock.patch.object(sc, "_enviar_al_proveedor", return_value="x = 1\n") as enviar,
            mock.patch.object(sc, "_validar_sintaxis") as val,
            mock.patch.object(agente, "sobrescribir", return_value=True) as sob,
        ):
            ok = agente._aplicar_modo_sobrescribir(
                archivo, "cambiar", "", modelo=None, directorio=self.tmp, validar=False
            )
        self.assertTrue(ok)
...
class TestPreviewParche(BaseValidacion):
    def test_preview_aplica_hunk(self):
        ...
        resultado, aplicados = agente._aplicar_parche_preview(parche, "def fn(): return 1\n")
