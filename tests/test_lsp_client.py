...
class TestDeteccion(unittest.TestCase):
    ...
    def test_extensiones_basicas(self):
        ...
        self.assertEqual(_detectar_lenguaje_por_extension("x.ts"), "typescript")
        self.assertEqual(_detectar_lenguaje_por_extension("x.jsx"), "javascript")
    ...
    def test_comando_servidor_instalado(self):
        ...
        with mock.patch.object(lc.shutil, "which", return_value="/usr/bin/pyright-langserver"):
...
class TestCache(unittest.TestCase):
    ...
    def test_guardar_y_obtener_memoria(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cache.guardar(str(archivo), 1, 1, "definicion", {"archivo": "mod.py", "linea": 3})
    ...
    def test_persistente_entre_instancias(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            r = CacheLSP(db_path=Path(tmp) / "c.db").obtener(str(archivo), 2, 3, "tipo")
...
class TestCliente(unittest.TestCase):
    ...
    def _cliente(self, tmp, timeout=2.0):
        cliente = LSPClient(cache=CacheLSP(db_path=Path(tmp) / "c.db"), timeout=timeout)
    ...
    def test_iniciar_sin_servidor_devuelve_false(self):
        cliente = LSPClient()
        with _silencio(), mock.patch.object(lc, "_comando_servidor", return_value=None):
    ...
    def test_iniciar_fallo_al_arrancar(self):
        ...
        with (
            _silencio(),
            mock.patch.object(lc, "_comando_servidor", return_value=["falso-servidor"]),
            mock.patch.object(cliente, "_arrancar_proceso", side_effect=OSError("no such file")),
        ):
            self.assertFalse(cliente.iniciar("python", "."))
    ...
    def test_enviar_peticion_error_rpc(self):
        ...
        cliente._cola.put({"jsonrpc": "2.0", "id": 1, "error": {"code": -32600, "message": "mal"}})
        self.assertIsNone(cliente.enviar_peticion("initialize", {}))
        cliente.cerrar()
    ...
    def test_obtener_definicion_mockeada(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cruda = {"uri": archivo.as_uri(), "range": {"start": {"line": 0, "character": 4}}}
            with _silencio(), mock.patch.object(cliente, "enviar_peticion", return_value=cruda):
    ...
    def test_obtener_referencias_mockeada(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cliente = self._cliente(tmp)
            ubicaciones = [
                {"uri": archivo.as_uri(), "range": {"start": {"line": 0, "character": 0}}},
                {"uri": archivo.as_uri(), "range": {"start": {"line": 3, "character": 4}}},
            ]
            with (
                _silencio(),
                mock.patch.object(cliente, "enviar_peticion", return_value=ubicaciones),
            ):
    ...
    def test_obtener_tipo_mockeado(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with _silencio(), mock.patch.object(cliente, "enviar_peticion", return_value=hover):
    ...
    def test_definicion_de_archivo_inexistente(self):
        cliente = LSPClient()
        with _silencio():
            self.assertIsNone(cliente.obtener_definicion("no_existe_xyz.py", 1, 1))
    ...
    def test_respuesta_vacia_devuelve_none(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cliente = self._cliente(tmp)
            with _silencio(), mock.patch.object(cliente, "enviar_peticion", return_value=None):
                self.assertIsNone(cliente.obtener_definicion(str(archivo), 1, 1))
            cliente.cerrar()
    ...
    def test_cache_del_cliente_evita_segunda_peticion(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            archivo.write_text("a\n", encoding="utf-8")
            cliente = self._cliente(tmp)
            cruda = {"uri": archivo.as_uri(), "range": {"start": {"line": 0, "character": 0}}}
            with (
                _silencio(),
                mock.patch.object(
                    cliente, "enviar_peticion", side_effect=lambda *a, **k: cruda
                ) as peticion,
            ):
...
class TestIntegracionReAct(unittest.TestCase):
    ...
    def test_herramientas_lsp_inactivas_error_claro(self):
        ...
        r = agente.herramientas["lsp_definicion"]({"archivo": "a.py", "linea": 1, "columna": 1})
    ...
    def test_lsp_definicion_via_cliente_mockeado(self):
        ...
        agente = ReactAgent(directorio=".", lsp=True)
        fake = mock.MagicMock()
        fake.obtener_definicion.return_value = {"archivo": "x.py", "linea": 3, "columna": 1}
        with _silencio(), mock.patch.object(lc, "obtener_cliente_lsp", return_value=fake):
            r = agente.herramientas["lsp_definicion"]({"archivo": "x.py", "linea": 2, "columna": 4})
    ...
    def test_lsp_no_disponible_error_claro(self):
        ...
        agente = ReactAgent(directorio=".", lsp=True)
        with _silencio(), mock.patch.object(lc, "obtener_cliente_lsp", return_value=None):
    ...
    def test_argumentos_invalidos(self):
        ...
        with (
            _silencio(),
            mock.patch.object(lc, "obtener_cliente_lsp", return_value=mock.MagicMock()),
        ):
            r = agente.herramientas["lsp_tipo"]({"archivo": "x.py", "linea": "uno", "columna": 1})
            self.assertFalse(r["ok"])
