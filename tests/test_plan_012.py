...
class TestNormalizarPasos(unittest.TestCase):
    def test_formato_envuelto(self):
        pasos = sc._normalizar_pasos(
            {
                "pasos": [
                    {"descripcion": "editar a.py", "accion": "editar", "archivos": ["a.py"]},
                    {
...
class TestGenerarPlan(unittest.TestCase):
    ...
    def test_generar_plan_con_openai_mock(self):
        ...
        plan_json = json.dumps(
            {"pasos": [{"descripcion": "paso 1", "accion": "ejecutar", "comando": "dir"}]}
        )
...
class TestGitPlan(unittest.TestCase):
    ...
    def test_no_es_repo_git_con_mock(self):
        with mock.patch.object(
            sc, "_ejecutar_comando", return_value=(128, "", "fatal: not a git repository")
        ):
            self.assertFalse(sc._es_repo_git(str(self.dir_tmp)))
        with mock.patch.object(sc, "_ejecutar_comando", return_value=(0, "true\n", "")):
    ...
    def test_commit_paso_sin_repo_es_ok(self):
        ...
        with (
            mock.patch.object(sc, "_es_repo_git", return_value=False),
...
class TestEjecutarPasoPlan(unittest.TestCase):
    ...
    def test_paso_consultar_con_proveedor_mock(self):
        with mock.patch.object(sc, "_enviar_al_proveedor", return_value="respuesta IA"):
    ...
    def test_paso_editar_usa_orquestador(self):
        ...
        self.assertEqual(orch._planificar.call_args[0][0].consulta, "arreglar login")
...
class TestEjecutarPlanificador(unittest.TestCase):
    def setUp(self):
        ...
        parches = [
            ...
            mock.patch.object(sc, "HISTORIAL_PATH", self.dir_tmp / "historial.json"),
        ]
        for p in parches:
            p.start()
    ...
    def test_sin_consulta_devuelve_1(self):
        self.assertEqual(sc._ejecutar_planificador(_args_base(consulta=None)), 1)
    ...
    def test_flujo_completo_con_mocks(self):
        ...
        with (
            ...
            mock.patch.object(sc, "_guardar_historial", side_effect=lambda e: guardados.append(e)),
        ):
    ...
    def test_cancelacion_por_el_usuario(self):
        with (
            mock.patch.object(
                sc, "_generar_plan", return_value=[{"descripcion": "p", "accion": "consultar"}]
            ),
...
class TestFlagsPlanCLI(unittest.TestCase):
    ...
    def test_git_commit_por_defecto_true(self):
        ...
        self.assertFalse(self._parse(["--plan", "x", "--no-git-commit"]).git_commit)
