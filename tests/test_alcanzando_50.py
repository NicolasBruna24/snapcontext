...
class TestCasos:
    def t1(self): assert isinstance(sc.VERSION, str)
    def t2(self): assert isinstance(sc.CARPETAS_DEFECTO, tuple)
    def t3(self): assert isinstance(sc.CARPETAS_PROYECTO_VALIDAS, (tuple, list))
    def t4(self): assert sc._contar_tokens("") == 0
    def t5(self): assert sc._contar_tokens("hola") >= 0
    def t6(self): assert sc._es_extension_python("a.py") is True
    def t7(self): assert sc._es_extension_python("a.js") is False
