...
class TestExtraerRazonamiento(unittest.TestCase):
    ...
    def test_dict_claves_alternativas(self):
        for clave in (
            ...
            self.assertEqual(sc._extraer_razonamiento({clave: "raz"}), "raz", clave)
    ...
    def test_str_bloques_think(self):
        ...
        self.assertEqual(sc._extraer_razonamiento(texto), "primero\nsegundo")
    ...
    def test_quitar_razonamiento_limpia_texto(self):
        limpio = sc._quitar_razonamiento("<think>oculto</think>Respuesta final")
        ...
        self.assertEqual(sc._quitar_razonamiento("<think>roto Respuesta"), "roto Respuesta")
...
class TestProcesarYPasos(unittest.TestCase):
    ...
    def test_procesar_inactivo_no_muestra_nada(self):
        with mock.patch.object(ui, "mostrar_razonamiento") as panel:
            limpio, raz = sc._procesar_razonamiento("<think>x</think>hola", activo=False)
    ...
    def test_procesar_activo_muestra_panel(self):
        with mock.patch.object(ui, "mostrar_razonamiento") as panel:
            limpio, raz = sc._procesar_razonamiento("<think>pienso</think>hola", activo=True)
    ...
    def test_dos_pasos_devuelve_razonamiento(self):
        with mock.patch.object(sc, "_enviar_al_proveedor", return_value="paso 1… paso 2…") as envio:
    ...
    def test_dos_pasos_extrae_think_de_respuesta(self):
        with mock.patch.object(sc, "_enviar_al_proveedor", return_value="<think>raz</think>final"):
            raz = sc._razonamiento_dos_pasos("t", "mockprov")
        self.assertEqual(raz, "raz")

    def test_dos_pasos_fallo_proveedor_devuelve_none(self):
        with mock.patch.object(sc, "_enviar_al_proveedor", side_effect=RuntimeError("boom")):
...
class TestPanelUI(unittest.TestCase):
    ...
    def test_texto_valido_devuelve_true(self):
        with (
            mock.patch.object(ui, "RICH_DISPONIBLE", False),
    ...
    def test_panel_rich_titulo_y_borde(self):
        with (
            mock.patch.object(ui, "RICH_DISPONIBLE", True),
...
class TestIntegracionEditor(unittest.TestCase):
    ...
    def test_activo_muestra_panel_y_limpia_respuesta(self):
        respuesta = "<think>Analizo la función saluda.</think>def saluda():\n    return 'adiós'\n"
        with (
    ...
    def test_inactivo_no_muestra_pero_limpia(self):
        respuesta = "<think>raz oculto</think>def saluda():\n    return 'chau'\n"
        with (
            mock.patch.object(sc, "_enviar_al_proveedor", return_value=respuesta),
            mock.patch.object(ui, "mostrar_razonamiento") as panel,
            mock.patch.object(sc, "info"),
...
class TestIntegracionChat(unittest.TestCase):
    ...
    def test_chat_activo_muestra_razonamiento(self):
        ...
        os.environ[ENV_CLAVE] = "1"
        respuestas = iter(["<think>Pienso la respuesta.</think>Respuesta final del modelo"])
        entradas = iter(["hola", "/salir"])
        with (
            mock.patch.object(
                sc, "_enviar_al_proveedor", side_effect=lambda *a, **k: next(respuestas)
            ),
            mock.patch("builtins.input", side_effect=lambda *a, **k: next(entradas)),
            mock.patch.object(ui, "mostrar_razonamiento") as panel,
            mock.patch.object(sc, "info"),
...
class TestIntegracionPlan(unittest.TestCase):
    ...
    def test_plan_limpia_razonamiento_y_parsea_json(self):
        ...
        fake_resp.choices[
            0
        ].message.content = '<think>razonando el plan</think>[{"accion": "consultar"}]'
        ...
        cfg = {
            "tipo": "openai",
            "nombre": "OpenAI de prueba",
            "modelo_default": "test-model",
            "clave_env": "SC_TEST_KEY",
            "requiere_clave": True,
        ...
        try:
            with (
                mock.patch.object(sc, "openai", fake_openai),
                mock.patch.dict(sc.PROVEEDORES, {"openai": cfg}),
                mock.patch.object(sc, "parsear_json", _parsear),
                mock.patch.object(sc, "_normalizar_pasos", side_effect=lambda p: p),
                mock.patch.object(sc, "info"),
                mock.patch.object(sc, "depurar"),
            ):
                sc._generar_plan("tarea", proveedor="openai", modelo="test-model")
        finally:
...
class TestIntegracionReact(unittest.TestCase):
    ...
    def _agente(self, activo):
        ...
        return react, react.ReactAgent(
            directorio=".", auto=True, max_iter=2, proveedor="ollama", mostrar_razonamiento=activo
        )

    def test_react_activo_muestra_razonamiento_completo(self):
        react, agente = self._agente(True)
        bruto = (
            "<think>Razonamiento paso a paso completo</think>"
            '{"pensamiento": "breve", "accion": "finalizar", '
            '"argumentos": {"resumen": "hecho"}}'
        )
        with (
            mock.patch.object(agente, "_llamar_llm", return_value=bruto),
        ...
        panel.assert_called_once()
        self.assertIn("Razonamiento paso a paso completo", panel.call_args.args[0])

    def test_react_inactivo_no_muestra_panel(self):
        react, agente = self._agente(False)
        bruto = (
            '{"pensamiento": "breve", "accion": "finalizar", "argumentos": {"resumen": "hecho"}}'
        )
        with (
            mock.patch.object(agente, "_llamar_llm", return_value=bruto),
            mock.patch("ui.mostrar_razonamiento") as panel,
