...
class TestEstructuraVsCode(unittest.TestCase):
    ...
    def test_package_json_valido(self):
        ...
        self.assertEqual(manifiesto["main"], "./extension/out/extension.js")

    def test_comandos_contribuidos(self):
        manifiesto = json.loads((VSCODE / "package.json").read_text("utf-8"))
        comandos = {c["command"] for c in manifiesto["contributes"]["commands"]}
        for esperado in (
    ...
    def test_menu_del_explorador(self):
        ...
        self.assertTrue(any(m["command"] == "snapcontext.anadirAlContexto" for m in menus))

    def test_configuraciones(self):
        manifiesto = json.loads((VSCODE / "package.json").read_text("utf-8"))
        propiedades = manifiesto["contributes"]["configuration"]["properties"]
        for esperada in (
            "snapcontext.pythonPath",
            "snapcontext.provider",
            "snapcontext.apiKey",
            "snapcontext.confirmar",
        ):
    ...
    def test_webview_es_copia_de_la_interfaz_web(self):
        web = (RAIZ / "web" / "static" / "index.html").read_text(encoding="utf-8")
        vscode_web = (VSCODE / "webview" / "index.html").read_text(encoding="utf-8")
        self.assertEqual(
            web, vscode_web, "vscode/webview/index.html debe copiar web/static/index.html"
        )

    def test_servidor_webview_reutiliza_web_app(self):
        codigo = (VSCODE / "webview" / "servidor_webview.py").read_text(encoding="utf-8")
