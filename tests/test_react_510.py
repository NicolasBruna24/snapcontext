...
def _dec(pensamiento="", accion="finalizar", argumentos=None):
    return {"pensamiento": pensamiento, "accion": accion, "argumentos": argumentos or {}}
...
class TestBucleBasico(BaseReact510):
    def test_bucle_completo_observa_y_finaliza(self):
        ...
        with (
            mock.patch.object(ra.ReactAgent, "_pedir_decision", side_effect=decisiones),
            mock.patch.object(
                ra.ReactAgent,
                "_tool_buscar_codigo",
                return_value={"ok": True, "total": 1, "coincidencias": ["m.py:1"]},
            ) as b,
            mock.patch.object(
                ra.ReactAgent,
                "_tool_leer_archivo",
                return_value={"ok": True, "ruta": "main.py", "contenido": "x"},
            ) as l,
        ):
    ...
    def test_historial_inicial_sistema_y_tarea(self):
        ...
        with mock.patch.object(
            ra.ReactAgent, "_pedir_decision", return_value=_dec(accion="finalizar")
        ):
    ...
    def test_finalizar_termina_limpio(self):
        ...
        with mock.patch.object(
            ra.ReactAgent,
            "_pedir_decision",
            return_value=_dec("acabamos", "finalizar", {"resumen": "hecho en 1 paso"}),
        ):
...
class TestLimitesYErrores(BaseReact510):
    def test_limite_de_iteraciones(self):
        ...
        with (
            mock.patch.object(
                ra.ReactAgent,
                "_pedir_decision",
                return_value=_dec("sigo", "buscar_codigo", {"patron": "x"}),
            ),
            mock.patch.object(ra.ReactAgent, "_tool_buscar_codigo", return_value={"ok": True}),
        ):
    ...
    def test_json_invalido_reintenta_correctivo(self):
        agente = self._agente()
        with mock.patch.object(
            ra.ReactAgent, "_llamar_llm", side_effect=["no es json", "tampoco", ""]
        ) as llamar:
            decision = agente._pedir_decision([{"role": "user", "content": "TAREA: t"}])
    ...
    def test_json_valido_tras_reintentos(self):
        ...
        with mock.patch.object(ra.ReactAgent, "_llamar_llm", side_effect=["basura", bruto]):
            decision = agente._pedir_decision([{"role": "user", "content": "TAREA: t"}])
    ...
    def test_accion_desconocida_se_reporta_al_llm(self):
        agente = self._agente()
        decisiones = [
            _dec("pruebo", "navegar_a_marte", {}),
            _dec("finalizo", "finalizar", {"resumen": "ok"}),
        ]
        with mock.patch.object(ra.ReactAgent, "_pedir_decision", side_effect=decisiones):
            res, salida = self._salida(agente.ejecutar, "t")
    ...
    def test_error_fatal_del_llm_aborta(self):
        agente = self._agente()
        with mock.patch.object(
            ra.ReactAgent, "_pedir_decision", side_effect=RuntimeError("API caída")
        ):
...
class TestHerramientas(BaseReact510):
    def test_editar_archivo_aplica_y_devuelve_diff(self):
        ...
        with mock.patch.object(
            ...
            side_effect=[
                _dec(
                    "creo",
                    "editar_archivo",
                    {"ruta": "lib/login.py", "contenido": "def login():\n    return True\n"},
                ),
                _dec("fin", "finalizar", {"resumen": "editado"}),
            ],
        ):
        ...
        obs = [m["content"] for m in agente.historial if m["content"].startswith("[OBSERVACIÓN")]
    ...
    def test_editar_archivo_bloquea_rutas_fuera_del_proyecto(self):
        ...
        r = agente._ejecutar_accion(
            "editar_archivo", {"ruta": "../../etc/passwd", "contenido": "malicioso"}
        )
    ...
    def test_ejecutar_pruebas_captura_el_fallo(self):
        ...
        with (
            mock.patch.object(ra.ReactAgent, "_pedir_decision", side_effect=decisiones),
            mock.patch.object(sc, "_ejecutar_comando", return_value=fallo) as ejecutar,
        ):
    ...
    def test_ejecutar_comando_usa_la_via_sandbox_aware(self):
        ...
        with (
            mock.patch.object(sc, "_SANDBOX_ACTIVO", False),
            mock.patch.object(sc, "_ejecutar_comando", return_value=(0, "hola\n", "")) as ej,
        ):
...
class TestInteractivoYContexto(BaseReact510):
    def test_modo_interactivo_pregunta_antes_de_actuar(self):
        ...
        with (
            mock.patch.object(ra.ReactAgent, "_pedir_decision", side_effect=decisiones),
            mock.patch.object(
                ra.ReactAgent, "_tool_buscar_codigo", return_value={"ok": True}
            ) as buscar,
            mock.patch("ui.preguntar_interactivo", return_value="c") as preguntar,
        ):
    ...
    def test_respuesta_abortar_detiene_el_bucle(self):
        ...
        with (
            mock.patch.object(
                ra.ReactAgent,
                "_pedir_decision",
                return_value=_dec("hago", "leer_archivo", {"ruta": "main.py"}),
            ),
            mock.patch("ui.preguntar_interactivo", return_value="a") as preguntar,
        ):
            res, _ = self._salida(agente.ejecutar, "t")
        self.assertFalse(res["ok"])
        self.assertTrue(res["abortado"])
    ...
    def test_en_auto_nunca_pregunta(self):
        ...
        decisiones = [
            _dec("paso", "buscar_codigo", {"patron": "x"}),
    ...
    def test_resumen_automatico_comprime_el_historial(self):
        ...
        with (
            mock.patch.object(ra, "_umbral_resumen", return_value=100),
            mock.patch.object(ra.ReactAgent, "_llamar_llm", return_value="Resumen corto.") as llm,
        ):
            cambio = agente._resumir_si_hace_falta()
        self.assertTrue(cambio)
        llm.assert_called_once()
        self.assertEqual(len(agente.historial), 2)
        self.assertIn("[RESUMEN DEL TRABAJO PREVIO]", agente.historial[1]["content"])
        self.assertNotIn("TAREA: mucha cosa", agente.historial[1]["content"])
...
class TestRouterModoPorDefecto(BaseReact510):
    ...
    def test_flag_react_equivale_a_default(self):
        args = sc.crear_parser().parse_args(["--react", '"refactoriza modulo.py"'])
