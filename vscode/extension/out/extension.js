...
exports.abrirChat = abrirChat;
exports.ejecutarConsulta = ejecutarConsulta;
exports.planificar = planificar;
exports.configurarApiKey = configurarApiKey;
exports.anadirAlContexto = anadirAlContexto;
exports.limpiarSeleccion = limpiarSeleccion;
...
function activate(context) {
    ...
    estadoBarra = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 10);
    ...
    const registros = [
        ...
        ["snapcontext.anadirAlContexto",
            (uri) => anadirAlContexto(uri)],
        ["snapcontext.limpiarSeleccion", () => limpiarSeleccion()],
    ];
    for (const [comando, callback] of registros) {
        const disposable = vscode.commands.registerCommand(comando, callback);
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("snapcontext.chat", new ChatViewProvider(), { webviewOptions: { retainContextWhenHidden: true } }));
...
class ChatViewProvider {
    resolveWebviewView(vista, _contexto, _token) {
        ...
        void (async () => {
            try {
                ...
                vista.webview.html = htmlWebview(ErrorView.HTML
...
ChatViewProvider.urlActiva = null;
...
async function abrirChat() {
    ...
    const panel = vscode.window.createWebviewPanel("snapcontextChat", "SnapContext Chat", vscode.ViewColumn.One, { enableScripts: true });
 ...
 */
async function arrancarServidorChat() {
    let puerto = 8765;
    ...
    const script = ("from web.app import arrancar_servidor; import threading, time;"
    ...
    procesoChat = (0, child_process_1.spawn)(pythonPath, ["-c", script], {
        cwd: ws,
        env: entornoConClave(),
        shell: false,
    });
    procesoChat.stdout?.on("data", (b) => canal().appendLine("[chat] " + b));
    procesoChat.stderr?.on("data", (b) => canal().appendLine("[chat] ⚠ " + b));
...
async function configurarApiKey() {
    ...
    await config().update("apiKey", clave, vscode.ConfigurationTarget.Workspace);
    vscode.window.showInformationMessage("SnapContext: API key guardada en la configuración del workspace.");
}
...
function anadirAlContexto(uri) {
    ...
    try {
        ...
        vscode.commands.executeCommand("setContext", "snapcontext.tieneSeleccion", archivosContexto.size > 0);
        vscode.window.showInformationMessage(`SnapContext: ${archivosContexto.size} archivo(s) en contexto.`);
...
function limpiarSeleccion() {
    ...
    vscode.commands.executeCommand("setContext", "snapcontext.tieneSeleccion", false);
    vscode.window.showInformationMessage("SnapContext: contexto de archivos vaciado.");
}
// ---------------------------------------------------------------------------
...
function ejecutarSnap(args, titulo) {
    return new Promise((resolve) => {
        ...
        const proc = (0, child_process_1.spawn)(pythonPath, argv, {
            cwd: ws,
            env: entornoConClave(),
            shell: false,
            windowsVerbatimArguments: false,
        });
        const pintar = (buf, esError = false) => {
        ...
        proc.stdout.on("data", (b) => pintar(b));
        ...
        proc.on("error", (err) => {
            canalSalida.appendLine(`✖ No se pudo lanzar Python (${pythonPath}): `
                + err.message);
            vscode.window.showErrorMessage("SnapContext: no se pudo lanzar Python. Revisa "
