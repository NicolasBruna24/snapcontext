...
def _ns_git(git_mensaje: str | None):
...
class ReactAgent:
    ...
    def __init__(  # noqa: C901  (refactor de complejidad: Fase 10c)
        ...
        self.max_historial = max(int(max_historial), 1) if max_historial else _max_historial()
        ...
        self._categoria_routing: str | None = None
    ...
    def _llamar_llm(self, mensajes: list[dict], timeout: float = LLM_TIMEOUT_SEG) -> str:
        with self._mutex:
            try:
                return asyncio.run(asyncio.wait_for(self._llamada_async(mensajes), timeout=timeout))
    ...
    # ------------------------------------------------------------------
    def _ruta_segura(self, ruta: str) -> Path | None:
    ...
    def _tool_ejecutar_comando(self, argumentos: dict) -> dict:
        ...
        codigo, stdout, stderr = sc._ejecutar_comando(comando, self.directorio, timeout=300)
        return {"ok": codigo == 0, "codigo": codigo, "stdout": stdout, "stderr": stderr}
    ...
    def _tool_browser(self, argumentos: dict, accion: str = "") -> dict:  # noqa: C901  (refactor de complejidad: Fase 10c)
        ...
        try:
            ...
            return {"ok": False, "error": f"mcp_tools_browser no disponible: {exc}"}
        ...
        if accion == "browser_abrir":
            return btool.browser_abrir(
                str(args.get("url", "")),
                wait_for=(str(args["wait_for"]) if args.get("wait_for") else None),
        ...
        if accion == "browser_type":
            return btool.browser_type(str(args.get("selector", "")), str(args.get("texto", "")))
        ...
        if accion == "browser_analizar_imagen":
            return btool.browser_analizar_imagen(
                str(args.get("imagen_base64", "")), str(args.get("pregunta", ""))
            )
        if accion == "browser_cerrar":
            return btool.browser_cerrar()
        return {"ok": False, "error": f"acción de navegador desconocida: {accion}"}
    ...
    def _tool_browser_analizar_imagen(self, argumentos: dict) -> dict:
        return self._tool_browser(argumentos, accion="browser_analizar_imagen")
    ...
    def _tool_invocar_sub_agente(self, argumentos: dict) -> dict:
        ...
        if not nombre:
            return {"ok": False, "error": "Falta el argumento 'nombre' del sub-agente."}
        ...
        try:
            ...
            return {"ok": False, "error": f"Sub-agente '{nombre}' no disponible: {exc}"}
    ...
    def _tool_lsp(self, argumentos: dict, accion: str = "") -> dict:
        ...
        if not self.lsp:
            return {
                "ok": False,
                "error": "LSP inactivo: arranca con --lsp o SNAPCONTEXT_LSP=1 para usar lsp_*.",
            }
        ...
        try:
            ...
            return {"ok": False, "error": "'linea'/'columna' deben ser enteros (1-based)."}
        if not archivo or linea < 1 or columna < 1:
            return {"ok": False, "error": "se requiere archivo, linea y columna (1-based)."}
        cliente = lc.obtener_cliente_lsp(self.directorio)
        if cliente is None:
            return {"ok": False, "error": "LSP no disponible para este lenguaje."}
        try:
            if accion == "lsp_definicion":
                r = cliente.obtener_definicion(archivo, linea, columna)
                if r:
                    sc.info(f"LSP: definición encontrada en {r['archivo']}:{r['linea']}")
    ...
    def _tool_buscar_codigo(self, argumentos: dict) -> dict:  # noqa: C901  (refactor de complejidad: Fase 10c)
        ...
        for camino in sorted(raiz.rglob("*")):
            ...
            if len(hallazgos) >= 50:
                break
        resultado = {"ok": True, "total": len(hallazgos), "coincidencias": hallazgos}
        # v5.5.0 (Graph RAG): añade archivos relacionados por dependencias.
        if self.graph_rag and hallazgos:
            afectados = sorted(
                {h.split(":", 1)[0] for h in hallazgos if h.split(":", 1)[0].endswith(".py")}
            )
    ...
    def _expander_con_grafo(self, archivos: list[str], max_adicionales: int = 3) -> list[str]:
        ...
        try:
            ...
            if not grafo:
                return archivos
            return gr.expandir_contexto(archivos, grafo, max_adicionales=max_adicionales)
        except Exception:
            return archivos
    ...
    def _pedir_decision(self, mensajes: list[dict]) -> dict | None:
        ...
        while intento < MAX_REINTENTOS_JSON:
            ...
            if decision is not None and ("accion" in decision or "action" in decision):
                accion = str(decision.get("accion") or decision.get("action") or "").strip()
                decision["accion"] = accion
                decision["pensamiento"] = str(
                    decision.get("pensamiento") or decision.get("thought") or ""
                )
                decision["argumentos"] = decision.get("argumentos") or decision.get("args") or {}
    ...
    def _observar_resultado(resultado: dict) -> str:
        ...
        for clave in ("ruta", "comando", "codigo", "total", "lineas", "url", "titulo"):
    ...
    def _tokens_historial(self) -> int:
        return sum(estimar_tokens(m.get("content", "")) for m in self.historial)
    ...
    def _comprimir_historial(self) -> None:
        ...
        pedido = [
            ...
            {"role": "user", "content": json.dumps(cuerpo, ensure_ascii=False)[:60000]},
        ]
        try:
    ...
    def ejecutar(self, consulta: str) -> dict:  # noqa: C901  (refactor de complejidad: Fase 10c)
        ...
        try:
            ...
            self._categoria_routing = _mr.clasificar_tarea(consulta, {"accion": "react"})
        except Exception:
        ...
        sandbox_txt = ", sandbox 🐳" if getattr(sc, "_SANDBOX_ACTIVO", False) else ""
        sc.info(
            f"🧠 Modo ReAct activado ({self.proveedor}, máx. "
        ...
        try:
            for iteracion in range(1, self.max_iteraciones + 1):
                ...
                _wi = self._wi
                if _wi is not None:
                    _wi.enviar_paso_react(iteracion, "pensamiento", decision.get("pensamiento", ""))
                    _wi.enviar_estado("Pensando...", decision.get("pensamiento", "")[:200])
                if self.mostrar_razonamiento:
                    # v6.2.0: razonamiento COMPLETO del modelo (no el resumido).
                    _raz = (
                        sc._extraer_razonamiento(getattr(self, "_ultima_respuesta_bruta", ""))
                        or decision["pensamiento"]
                    )
                    ui.mostrar_razonamiento(_raz)
                sc.info(f"[{iteracion}/{self.max_iteraciones}] 💭 {decision['pensamiento'][:200]}")
                if accion == "finalizar":
                    resumen = str(
                        decision["argumentos"].get("resumen", "")
                        or decision["pensamiento"]
                        or "Tarea finalizada."
                    )
                ...
                if not self.auto:
                    ...
                    if respuesta == "s":
                        ...
                        continue
                resultado_accion = self._ejecutar_accion(accion, decision["argumentos"])
                if self._wi is not None:
                    self._wi.enviar_paso_react(
                        iteracion,
                        "accion",
                        accion,
                        argumentos=json.dumps(decision.get("argumentos", {}), ensure_ascii=False)[
                            :2000
                        ],
                    )
                ...
                if resultado_accion is None:
                    observacion = (
                        f"ACCION DESCONOCIDA: '{accion}'. Válidas: "
                ...
                if ok_accion and self.git_commit and accion in self.ACCIONES_CON_COMMIT:
                    try:
                        _hash = sc._commit_paso(
                            {
                                "accion": accion,
                                "descripcion": str(decision.get("pensamiento") or accion),
                            },
                            _ns_git(self.git_mensaje),
                            self.directorio,
                        )
                        if _hash:
                            self.historial.append({"role": "user", "content": f"[COMMIT] {_hash}"})
                ...
                if self._wi is not None:
                    self._wi.enviar_paso_react(
                        iteracion, "error" if not ok_accion else "observacion", observacion[:4000]
                    )
                self.historial.append(
                    {"role": "assistant", "content": json.dumps(decision, ensure_ascii=False)}
                )
                self.historial.append(
                    {"role": "user", "content": f"[OBSERVACIÓN de '{accion}']\n{observacion}"}
                )
                ...
                if self._resumir_por_longitud():
                    sc.info(
                        f"[ReAct] Historial resumido tras superar "
            ...
            try:
                ...
                _hooks.ejecutar_hook("session_end", {"modo": "react", "consulta": consulta})
            except Exception:
                pass
    ...
    def _preguntar_continuar(self, decision: dict) -> str:
        ...
        return preguntar_interactivo(
            [("c", "Continuar"), ("a", "Abortar"), ("s", "Saltar esta acción")],
    ...
    # ------------------------------------------------------------------
    def _ejecutar_accion(self, accion: str, argumentos: dict) -> dict | None:
