...
def _metadatos_tree_sitter(contenido: str, lenguaje: str) -> list[dict] | None:
...
# ---------------------------------------------------------------------------
def _metadatos_ast(contenido: str) -> list[dict] | None:
...
_REGEX_FUNCIONES = {
    ...
    "javascript": re.compile(r"^(?:export\s+)?(?:async\s+)?function(?:\s+\*)?\s+([A-Za-z_$]\w*)"),
    "typescript": re.compile(r"^(?:export\s+)?(?:async\s+)?function(?:\s+\*)?\s+([A-Za-z_$]\w*)"),
    "tsx": re.compile(r"^(?:export\s+)?(?:async\s+)?function(?:\s+\*)?\s+([A-Za-z_$]\w*)"),
    "go": re.compile(r"^func(?:\s+\([^)]*\))?\s+([A-Za-z_]\w*)"),
    "rust": re.compile(r"^(?:pub(?:\s*\([^)]*\))?\s+)?(?:unsafe\s+)?fn\s+([A-Za-z_]\w*)"),
    "java": re.compile(
...
_REGEX_CLASES = {
    ...
    "php": re.compile(r"^\s*(?:abstract\s+|final\s+)?(?:class|interface)\s+([A-Za-z_]\w*)"),
    "swift": re.compile(
        r"^\s*(?:private\s+|public\s+|internal\s+|open\s+)?"
        r"class\s+([A-Za-z_]\w*)"
    ),
    "c": re.compile(r"^\s*(?:typedef\s+)?(?:struct|enum|union)\s+([A-Za-z_]\w*)"),
    "cpp": re.compile(r"^\s*(?:class\s+|struct\s+|enum\s+|union\s+)([A-Za-z_]\w*)"),
...
def _regex_para(lenguaje: str) -> tuple[re.Pattern | None, re.Pattern | None]:
    """Devuelve las regex (función, clase) para ``lenguaje``."""
    clave = (lenguaje or "").strip().lower()
    return (
        _REGEX_FUNCIONES.get(clave) or _REGEX_FUNCION_GENERICA,
...
def _metadatos_regex(contenido: str, lenguaje: str) -> list[dict]:
    ...
    for k, (i, _ind, tipo, nombre) in enumerate(hits):
        ...
        while fin > i and not lineas[fin].strip():
            fin -= 1
        metadatos.append({"nombre": nombre, "tipo": tipo, "inicio": i + 1, "fin": fin + 1})
    return metadatos
...
def _texto_resumen(contenido: str, lenguaje: str, metadatos: list[dict]) -> str:
...
def _seleccionar_bloques(
    metadatos: list[dict], objetivo: str | None, lineas: list[str], presupuesto: int
) -> list[dict]:
    ...
    if indice is not None:
        ...
        resto.sort(key=lambda m: min(abs(m["inicio"] - ancla), abs(m["fin"] - ancla)))
    else:
...
def seleccionar_contexto(
    ...
    if not metadatos:
        ...
        presupuesto = max_tokens - estimar_tokens("\n".join(partes))
        for m in _seleccionar_bloques(metadatos, objetivo, lineas, presupuesto):
            inicio, fin = m["inicio"], m["fin"]
            if inicio > fin:
                inicio, fin = fin, inicio
            inicio = max(inicio, 1)
            fin = min(fin, total)
            partes.append(
                f"# ── {m['tipo'].title()} {m['nombre']} (líneas {inicio}-{fin} de {total}) ──"
            )
...
def objetivo_en_mensaje(contenido: str, lenguaje: str, mensaje: str) -> str | None:
