...
- PestaÃ±as: Logs, Control, Diffs; Ã¡rbol de archivos del proyecto.
- ActualizaciÃ³n en tiempo real vÃ­a la cola de ``tui_hub`` (no bloqueante).

Si Textual no estÃ¡ instalado, la importaciÃ³n degrada y ``--tui`` muestra un
...
except Exception:  # pragma: no cover - degradaciÃ³n sin Textual
...
def _escapar(linea: str) -> str:
    """Escapa el marcado Rich de una lÃ­nea de diff."""
...
if TEXTUAL_DISPONIBLE:
    ...
    class SnapContextTUI(App):  # type: ignore[misc]
        ...
        Consume la cola de ``tui_hub`` de forma asÃ­ncrona y muestra logs,
        ...
        BINDINGS = [
            ...
            ("ctrl+t", "toggle_arbol", "Ãrbol"),
            ("ctrl+d", "limpiar_diffs", "Limpiar diffs"),
        ]

        def __init__(
            self, consulta: str = "", tarea: threading.Thread | None = None, version: str = "6.23.0"
        ) -> None:
            super().__init__()
            ...
            self.sub_title = "TUI inmersiva - Ctrl+Q salir Â· Ctrl+L limpiar logs"
                ...
                with Vertical(id="derecha"):
                    with TabbedContent(initial="tab-logs"):
                        ...
                        with TabPane("Diffs", id="tab-diffs"):
                            yield RichLog(id="diffs", highlight=True, markup=True, wrap=True)
            yield Footer()
        ...
        def refrescar_cola(self) -> None:
            ...
            if cola is None:
                return
            drenados = 0
            while drenados < 200:  # lÃ­mite por tick para no bloquear la UI
                try:
        ...
        def _procesar_evento(self, evento: dict) -> None:
            ...
            if tipo == "log":
                self._agregar_log(str(evento.get("nivel", "info")), str(evento.get("texto", "")))
            elif tipo == "react_step":
                try:
                    self._pasos = max(self._pasos, int(evento.get("iteracion", 0) or 0))
                ...
                self._refrescar_estado(
                    str(evento.get("estado", "inactivo")), str(evento.get("detalle", ""))
                )
            elif tipo == "diff":
                self._agregar_diff(str(evento.get("ruta", "")), str(evento.get("diff", "")))
            elif tipo == "fin":
                ok = bool(evento.get("ok", True))
                self._refrescar_estado("inactivo" if ok else "error")
                self._agregar_log(
                    "info" if ok else "error", f"Agente finalizado. {evento.get('resultado', '')}"
                )

        def _agregar_log(self, nivel: str, texto: str) -> None:
            self.query_one("#logs", RichLog).write(f"{_texto_nivel(nivel)} {texto}")
        ...
        def on_button_pressed(self, evento) -> None:
            ...
            if boton == "btn-pausar":
                ...
                log.write("[red]CancelaciÃ³n solicitada.[/]")
...
else:  # pragma: no cover - Textual no instalado: stub para importaciÃ³n segura

    class SnapContextTUI:  # type: ignore[no-redef]
        """Stub que permite importar el mÃ³dulo sin Textual instalado."""

        def __init__(self, *args, **kwargs) -> None:
            raise RuntimeError(
                "Textual no estÃ¡ instalado. InstÃ¡lalo con: "
...
# ------------------------------------------------------------- punto entrada --
def ejecutar_tui(
    consulta: str = "", tarea: threading.Thread | None = None, version: str = "6.20.0"
) -> int:
    """Lanza la TUI de SnapContext. Devuelve el cÃ³digo de salida.

    - Si Textual no estÃ¡ instalado, imprime un error claro y devuelve 2.
    - ``tarea`` es el hilo que ejecuta al agente; se arranca al montar la app.
    """
    if not TEXTUAL_DISPONIBLE:
        print("Textual no estÃ¡ instalado. Instala el grupo opcional:")
