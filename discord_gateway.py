...
def obtener_public_key() -> str | None:
...
def obtener_application_id() -> str | None:
...
def obtener_bot_token() -> str | None:
...
def guardar_configuracion_discord(
    public_key: str | None,
...
# ---------------------------------------------------------------------------
def verify_signature(data: bytes, signature_hex: str, timestamp: str) -> bool:
    ...
    if Ed25519PublicKey is None:
        raise ValueError("Falta 'cryptography': pip install cryptography")
    ...
    try:
        Ed25519PublicKey.from_public_bytes(bytes.fromhex(public_key_hex)).verify(firma, mensaje)
    ...
    try:
        async with httpx.AsyncClient(timeout=60) as cliente:
            if interaction_token:
                ...
                if not webhook_url:
                    print("⚠ [discord] Sin webhook ni interaction_token: no se puede responder.")
                    return False
                base = webhook_url
            ...
            await cliente.post(base, json={"content": cabeza + "\n… (salida completa adjunta)"})
            archivos = {
                "files[0]": ("snapcontext_salida.txt", content.encode("utf-8"), "text/plain")
            }
...
# ---------------------------------------------------------------------------
def enviar_notificacion(canal_id_o_webhook: str | None, mensaje: str) -> bool:
    ...
    try:
        ...
        return asyncio.run(
            send_discord_message(canal_id_o_webhook or obtener_webhook_url(), mensaje)
        )
    except Exception as exc:
...
async def run_agent_async(query: str) -> str:
    ...
    if texto.startswith("/"):  # compat.: admite "/fix tarea"
        ...
        return await loop.run_in_executor(None, _ejecutar_pipeline, texto, argv_extra)
    return await loop.run_in_executor(None, _ejecutar_pipeline, texto, [])


_TAREAS_ACTIVAS: set = set()


async def handle_discord_interaction(interaction_data: dict) -> dict | None:
    ...
    canal = (interaction_data.get("channel") or {}).get("id") or interaction_data.get("channel_id")
    ...
    tarea = asyncio.create_task(
        _procesar_y_responder(
            canal, comando, consulta, argv_extra, interaction_id, interaction_token
        )
    )
...
async def _procesar_y_responder(  # noqa: C901  (refactor de complejidad: Fase 10c)
    ...
    if comando == "pr":
        ...
        try:
            ...
            if ok:
                ...
                respuesta = (
                    f"⚠️ No se pudo cancelar la tarea `#{tid}` (no existe o ya no está pendiente)."
                )
        except Exception as exc:
    ...
    await send_discord_message(
        None, respuesta, interaction_id=interaction_id, interaction_token=interaction_token
    )
