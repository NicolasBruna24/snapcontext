...
COMANDOS_SERVIDOR: dict[str, list[list[str]]] = {
    ...
    "javascript": [["typescript-language-server", "--stdio"], ["javascript-typescript-stdio"]],
...
def _comando_servidor(lenguaje: str) -> list[str] | None:
...
class CacheLSP:
    ...
    def obtener(self, archivo: str, linea: int, columna: int, tipo: str) -> dict | None:
    ...
    def guardar(self, archivo: str, linea: int, columna: int, tipo: str, respuesta: dict) -> None:
        clave = self._clave(archivo, linea, columna, tipo)
        hash_actual = _hash_archivo(archivo)
        with self._mutex:
            self.memoria[clave] = {"hash": hash_actual, "respuesta": dict(respuesta)}
        if not self.db_path:
            return
        try:
            con = sqlite3.connect(str(self.db_path))
    ...
    def invalidar(self, archivo: str, linea: int, columna: int, tipo: str) -> None:
        clave = self._clave(archivo, linea, columna, tipo)
        with self._mutex:
        ...
        try:
            ...
            with con:
                con.execute(
                    "DELETE FROM lsp_cache WHERE clave = ?",
                    (json.dumps(list(clave), ensure_ascii=False),),
                )
            con.close()
        except sqlite3.Error:
...
class LSPClient:
    ...
    def __init__(self, cache: CacheLSP | None = None, timeout: float = TIMEOUT_DEFECTO) -> None:
        self.lenguaje: str | None = None
        self.ruta_proyecto: str | None = None
    ...
    # -- Ciclo de vida -----------------------------------------------------
    def iniciar(self, lenguaje: str | None = None, ruta_proyecto: str = ".") -> bool:
        ...
        if not comando:
            sc.aviso(
                f"LSP no disponible para este lenguaje ({lenguaje}): "
    ...
    def _arrancar_proceso(self, comando: list[str], ruta_proyecto: str) -> None:
        ...
        self.proceso = subprocess.Popen(
            ...
            cwd=str(Path(ruta_proyecto).resolve()),
            bufsize=0,
        )
        self._lector = threading.Thread(target=self._bucle_lector, daemon=True)
        self._lector.start()
    ...
    def _inicializar_sesion(self) -> bool:
        ...
        respuesta = self.enviar_peticion(
            "initialize",
            {
                "processId": None,
    ...
    def enviar_notificacion(self, metodo: str, params: dict) -> None:
        ...
        self._enviar_mensaje({"jsonrpc": "2.0", "method": metodo, "params": params})

    def enviar_peticion(
        self, metodo: str, params: dict, usar_cache: bool = False, clave_cache: tuple | None = None
    ) -> dict | None:
        ...
        self._enviar_mensaje(
            {"jsonrpc": "2.0", "id": peticion_id, "method": metodo, "params": params}
        )
    ...
    def _preparar_archivo(self, archivo: str) -> dict | None:
    ...
    def obtener_definicion(self, archivo: str, linea: int, columna: int) -> dict | None:
    ...
    def obtener_referencias(self, archivo: str, linea: int, columna: int) -> dict | None:
        """Todas las referencias al símbolo (incluida la declaración).

        Devuelve ``{referencias: [...], total}`` o ``None``.
        """
        return self._consulta_posicional(
            "textDocument/references", archivo, linea, columna, "referencias", contexto=True
        )

    def obtener_tipo(self, archivo: str, linea: int, columna: int) -> dict | None:
        """Tipo de la variable/expresión (hover). Devuelve ``{tipo}``."""
        return self._consulta_posicional("textDocument/hover", archivo, linea, columna, "tipo")

    def _consulta_posicional(
        self, metodo: str, archivo: str, linea: int, columna: int, tipo: str, contexto: bool = False
    ) -> dict | None:
    ...
    @staticmethod
    def _parsear_respuesta(respuesta: dict | None, tipo: str) -> dict | None:
        ...
        if tipo == "tipo":
            ...
            if isinstance(contenido, dict):
                ...
                partes = [p.get("value", "") if isinstance(p, dict) else str(p) for p in contenido]
        ...
        if isinstance(respuesta, list):
            ...
            if "targetUri" in primero:
                primero = {
                    "uri": primero.get("targetUri", ""),
                    "range": (
                        primero.get("targetSelectionRange") or primero.get("targetRange") or {}
                    ),
                }
...
# ---------------------------------------------------------------------------
_CLIENTE_GLOBAL: LSPClient | None = None
...
def cliente_lsp_activo(flag: bool | None = None) -> bool:
...
def obtener_cliente_lsp(ruta_proyecto: str = ".", lenguaje: str | None = None) -> LSPClient | None:
