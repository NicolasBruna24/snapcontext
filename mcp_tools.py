...
def _ejecutar_herramienta_mcp(nombre: str, argumentos: Optional[dict] = None,
    ...
    if cfg.get("requiere_permiso"):
        ...
        if not _sc._confirmar_accion(f"usar herramienta '{nombre}'",
    ...
    try:
        if nombre == "grep":
            ...
            resultado = _sc._tool_git_diff(str(argumentos.get("directorio", ".")),
            ...
            resultado = _sc._estado_proceso_fondo(_entero_opcional(
                argumentos.get("pid")))
    ...
    try:
        _sc._hooks.ejecutar_hook("after_tool_use", {
