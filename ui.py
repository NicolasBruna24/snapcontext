...
REPO_URL = os.environ.get("SNAPCONTEXT_REPO", "https://github.com/NicolasBruna24/snapcontext")
...
def mostrar_tabla_impacto(
    dependencias: dict[str, Sequence[str]], criticas: set | None = None
) -> None:
    ...
    for archivo, deps in dependencias.items():
        ...
        for dep in deps:
            tabla.add_row(
                ...
                Text("Revisar y actualizar los usos tras el cambio", style="dim"),
            )
    _console.print(tabla)


def mostrar_diff(archivo: str, lines_added: int, lines_removed: int, contexto: str) -> None:
    ...
    try:
        _console.print(
            Syntax(contexto or "(sin cambios)", "diff", theme="monokai", line_numbers=False)
        )
    except Exception:  # pragma: no cover - blindaje UI
        _console.print(contexto or "(sin cambios)")


def preguntar_interactivo(
    opciones: list[tuple[str, str]] | None, mensaje: str, defecto: str = "c"
) -> str:
...
def mostrar_razonamiento(
    texto: str, titulo: str = "🧠 Razonamiento del modelo", max_caracteres: int = 500
) -> bool:
    ...
    if RICH_DISPONIBLE:
        ...
        if truncado:
            cuerpo.append(
                f"\n\n[dim][ver más] razonamiento truncado a {max_caracteres} caracteres[/dim]"
            )
        _console.print(
            Panel(cuerpo, title=titulo, border_style="green", subtitle="chain-of-thought")
        )
    else:
        ...
        _imprimir(visible)
        if truncado:
            _imprimir(f"[ver más] razonamiento truncado a {max_caracteres} caracteres")
