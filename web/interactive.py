...
_CALLBACK_LIBRE: Callable[[dict], None] | None = None
...
def enviar_estado(estado: str, detalle: str = "") -> bool:
    ...
    return emitir(
        "agent_status", estado=str(estado)[:120], detalle=str(detalle or "")[:MAX_CONTENIDO]
    )
...
def enviar_log(nivel: str, texto: str) -> bool:
    ...
    if nivel not in ("info", "warning", "error"):
        nivel = "info"
    return emitir("log_interactivo", nivel=nivel, texto=str(texto or "")[:MAX_CONTENIDO])


# --------------------------------------------------------------------------
# Resolución visual de conflictos de parche (diff viewer)
# --------------------------------------------------------------------------
def enviar_conflicto_diff(
    ruta: str, original: str, propuesto: str, timeout: int = 600
) -> dict[str, Any] | None:
...
def registrar_callback_libre(callback: Callable[[dict], None] | None) -> None:
...
__all__ = [
    ...
    "desactivar",
    "emitir",
    "enviar_conflicto_diff",
    "enviar_estado",
    "enviar_log",
    "enviar_paso_react",
    "esta_activo",
    "registrar_callback_libre",
    "reiniciar",
]
