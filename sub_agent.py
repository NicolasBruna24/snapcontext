...
ROLES: dict[str, dict[str, Any]] = {
    ...
    "documentador": {
        ...
        "herramientas": ["leer_archivo", "buscar_codigo", "editar_archivo", "finalizar"],
        "max_iter": 8,
    },
    "reviewer": {
        "emoji": "REVIEWER",
...
class SubAgentRegistry:
    ...
    def registrar(self, nombre: str, config: dict[str, Any]) -> None:
        ...
        cfg.setdefault("herramientas", ["leer_archivo", "buscar_codigo", "finalizar"])
...
class SubAgente:
    ...
    def __init__(
        ...
        if config and isinstance(config, dict):
            self.config_rol = {
                ...
                "prompt": str(config.get("prompt") or ROLES.get(rol, {}).get("prompt", "")),
                "herramientas": list(
                    config.get("herramientas")
                    or ROLES.get(rol, {}).get("herramientas", ["leer_archivo", "finalizar"])
                ),
                "max_iter": int(config.get("max_iter") or ROLES.get(rol, {}).get("max_iter", 8)),
            }
        else:
            rol = str(rol).strip().lower()
            if rol not in ROLES:
                raise ValueError(
                    f"Rol de sub-agente desconocido: '{rol}'. Válidos: {', '.join(ROLES_VALIDOS)}"
                )
    ...
    # Comunicación
    # ------------------------------------------------------------------
    def enviar_mensaje(self, mensaje: Any, remitente: str = "supervisor") -> None:
        """Recibe un mensaje de otro agente (buzón de entrada)."""
        with self._mutex:
            self._entrada.append({"remitente": remitente, "contenido": mensaje})
    ...
    def ejecutar(self, consulta: str) -> dict:
        ...
        if self.buzon is not None:
            try:
                ...
                pass
        if resultado.get("ok"):
            sc.exito(f"✅ Sub-agente {self.nombre} completado: {resultado.get('resultado')}")
        else:
...
def ejecutar_sub_agentes_paralelo(
    especificaciones: list[dict], max_parallel: int = 3, buzon: Any | None = None
) -> list[dict]:
    ...
    resultados: list[dict | None] = [None] * len(especificaciones)
    if not especificaciones:
        return []
    if len(especificaciones) > 1:
        sc.info(
            f"Ejecutando {len(especificaciones)} sub-agentes en "
    ...
    hilos = [
        threading.Thread(target=_trabajador, args=(i, espec), daemon=True)
